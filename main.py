...
def open_folder(c):
    ...
    if cat == "paid_service" and not owns_item:
        ...
        return raw_bot.send_message(uid, _service_card(folder) + "\n\nPaid products are purchased separately by every user, including VIP members.", reply_markup=kb)
...
def upload_file_step(m, cat, uid, files, is_service):
    ...
    if m.text == "/done":
        ...
        return
    if m.content_type in ["document", "photo", "video", "audio", "voice"]:
    ...
    if approved:
        ...
        try:
            ...
            if folder:
                ...
                raw_bot.send_message(row["user_id"], "✅ Payment approved. Your product is unlocked in Services.")
        except Exception as exc:
