...
def premium_emoji_menu(m):
    ...
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("🔗 Add Emoji Pack Link", callback_data="premiumemoji|pack"))
    ...
    raw_bot.send_message(m.from_user.id, f"✨ PREMIUM EMOJIS\n\nSaved mappings: {count}\n\nAdd a Telegram custom emoji pack link (t.me/addemoji/...) or send custom premium emojis directly. The bot maps each premium emoji to its normal emoji meaning and automatically uses it in generated proof/status messages. Auto Posts already preserve the exact premium emojis from the post you send or forward.", reply_markup=kb)
...
def premium_emoji_cb(c):
    ...
    prompt = "Send the Telegram custom emoji pack link, for example: https://t.me/addemoji/PackName" if action == "pack" else "Send a message containing the premium custom emojis you want the bot to learn."
...
def premium_emoji_pack_step(m):
    try:
        value = (m.text or "").strip()
        match = re.search(r"(?:https?://)?t\.me/addemoji/([A-Za-z0-9_]+)", value, re.I)
        if not match:
            raise ValueError("Send a valid t.me/addemoji/... link")
        ...
        for sticker in getattr(sticker_set, "stickers", []) or []:
            ...
            if glyph and custom_id:
                ...
                added += 1
        if not added:
            raise ValueError("No custom emoji mappings were found in that pack")
        set_config("premium_emoji_map", mapping)
        raw_bot.send_message(m.chat.id, f"✅ Premium emoji pack added. Learned {added} emoji entries. Total saved: {len(mapping)}", reply_markup=admin_menu())
    except Exception as exc:
        admin_error(m.from_user.id, exc)
...
def _build_post_maker_text(details, variant=0):
    ...
    styles = [
        f"🔥 {title} 🔥\n\n{body}\n\n✨ Clear. Useful. Easy to follow.\n🚀 Don't miss the update!",
        f"🚨 {title} 🚨\n\n{body}\n\n💎 Everything you need in one place.\n📌 Check it out now!",
        f"✨ {title} ✨\n\n{body}\n\n📚 Simple guidance • ⚡ Quick steps\n🔥 Stay updated with GLOBEXOMART.",
        f"💥 {title} 💥\n\n{body}\n\n✅ Useful info • ✅ Full guidance\n🚀 Take action while it's available.",
        f"👑 {title} 👑\n\n{body}\n\n💡 Learn it. Use it. Grow.\n🔥 More updates from GLOBEXOMART.",
    ]
