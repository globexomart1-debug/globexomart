...
def open_catalog_menu(m):
    ...
    if m.text == "📚 Methods":
        ...
        kb.add(
            InlineKeyboardButton("🆓 Free Products", callback_data="catalog|free_service"),
            InlineKeyboardButton("💵 Paid Products", callback_data="catalog|paid_service"),
        )
...
def upload_file_step(m, cat, uid, files, is_service):
    ...
    if m.content_type in ["document", "photo", "video", "audio", "voice"]:
        ...
        prompt = raw_bot.send_message(uid, f"✅ Saved ({len(files)} file(s)). Send another file, or /done when finished.")
    else:
        prompt = raw_bot.send_message(uid, "Send a supported file/photo/video/audio, or /done when finished.")
...
@bot.message_handler(func=lambda m: m.text in ["📦 Upload Free Method", "💎 Upload VIP Method", "🆓 Upload Free Product", "💵 Upload Paid Product"] and is_admin(m.from_user.id))
def upload_handler(m):
    cats = {
        "📦 Upload Free Method": ("free", False),
        "💎 Upload VIP Method": ("vip", False),
        "🆓 Upload Free Product": ("free_service", True),
        "💵 Upload Paid Product": ("paid_service", True),
    }
