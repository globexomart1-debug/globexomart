...
def buy_vip_button(m):
    ...
    if current:
        ...
        plan_name = (get_subscription_plans().get(current.get("plan"), {}) or {}).get("name", current.get("plan"))
    ...
    for code, row in plans.items():
        if not row.get("active", True):
            continue
        price = _effective_plan_price(uid, code) if "_effective_plan_price" in globals() else float(row.get("price", 0))
        label = str(row.get("name") or code)
        kb.add(InlineKeyboardButton(f"💎 {label} • {int(row.get('days',0))} days • ${price:g}", callback_data=f"subplan|{code}"))
    if "_eligible_trial_offer" in globals() and _eligible_trial_offer(uid):
    ...
    for code, row in plans.items():
        if row.get("active", True):
            plan_lines.append(f"• {row.get('name', code)} — {int(row.get('days',0))} days — ${_effective_plan_price(uid, code) if '_effective_plan_price' in globals() else float(row.get('price',0)):g}")
    raw_bot.send_message(
        uid,
        f"{intro}\n\n💳 VIP PLANS\n" + ("\n".join(plan_lines) or "No active plans") +
        "\n\n✅ Admin controls every plan name, price and interval.\n"
        "⏳ VIP and private channel/group access are removed automatically when the interval ends.",
        reply_markup=kb,
    )
...
def vip_settings_menu(m):
    ...
    for code, row in get_subscription_plans().items():
        ...
        kb.add(
            InlineKeyboardButton(f"📅 {row.get('name',code)} Days", callback_data=f"vipplandays|{code}"),
...
def view_vip_plans_cb(c):
    ...
    lines = ["⚙️ VIP PLANS"]
    for code, row in plans.items():
        lines.append(f"{row.get('name',code)} [{code}]: ${row['price']:g} USDT • {row['days']} days • {'ON' if row.get('active',True) else 'OFF'}")
...
def get_subscription_plans():
    ...
    if normalized:
        return normalized
    return {k: {**v, "name": k, "active": True} for k, v in SUBSCRIPTION_PLANS.items()}
...
def _vip_plan_add_save(m):
    try:
        ...
        name, days, price = parts[0][:40], int(parts[1]), float(parts[2])
        if not name or days < 1 or price < 0: raise ValueError("Invalid values")
        plans = get_config().get("subscription_plans") or {}
        code = "P" + str(int(time.time()*1000))[-10:]
        plans[code] = {"name": name, "days": days, "price": price, "active": True}
        set_config("subscription_plans", plans)
        raw_bot.send_message(m.chat.id, f"✅ Added {name}: {days} days, ${price:g} USDT.", reply_markup=admin_menu())
    except Exception as exc:
