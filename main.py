...
def attach_method_pick_cb(c):
    ...
    try:
        ...
        try:
            raw_bot.send_message(pending.get("source_chat"), f"✅ Added to method: {folder.get('name')}")
        except Exception:
            pass
...
def _queue_auto_import(m, edited=False):
    ...
    try:
        ...
        if part:
            # For #methodN.part / #addN / #partN, let admins choose ANY existing method.
            # This is safer than relying only on the numeric tag and matches the admin workflow.
            pending_doc = {
                "cat": cat,
            ...
            pending_id = pending_methods_col.insert_one(pending_doc).inserted_id
            _send_method_attachment_picker(pending_id, cat)
            if source_reply_enabled:
                raw_bot.send_message(m.chat.id, "✅ Method part received. Admin can now choose which existing method to add it to.")
            return
