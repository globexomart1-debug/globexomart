...
def _invoice_pdf_escape(value):
    """Escape text for a basic PDF text object using built-in fonts."""
    text = str(value if value is not None else "")
    # Built-in Helvetica is Latin-1-ish; replace unsupported characters safely.
    text = text.encode("latin-1", "replace").decode("latin-1")
    return text.replace("\\", "\\\\").replace("(", "\\(").replace(")", "\\)")


def _make_invoice_pdf_bytes(title, invoice_id, rows, status="Paid / Delivered Successfully"):
    """Create a clean one-page GLOBEXOMART invoice without external PDF libraries."""
    from io import BytesIO

    # A4 points: 595 x 842. Build a simple professional invoice using PDF primitives.
    content = []
    def txt(x, y, size, text, font="F1"):
        content.append(f"BT /{font} {size} Tf {x} {y} Td ({_invoice_pdf_escape(text)}) Tj ET")
    def line(x1, y1, x2, y2, width=1):
        content.append(f"{width} w {x1} {y1} m {x2} {y2} l S")
    def fill_rect(x, y, w, h, r, g, b):
        content.append(f"{r} {g} {b} rg {x} {y} {w} {h} re f 0 0 0 rg")

    # Header / brand.
    fill_rect(35, 690, 525, 115, 0.07, 0.25, 0.55)
    txt(205, 765, 22, "GLOBEXOMART", "F2")
    txt(190, 742, 10, "Premium Methods, Digital Products & VIP Access")
    txt(229, 721, 9, "Official Purchase Invoice")
    txt(228, 703, 9, "@globexomartbot")

    # Invoice ID bar.
    fill_rect(75, 645, 445, 30, 0.10, 0.36, 0.70)
    txt(205, 655, 11, f"Invoice ID: #{invoice_id}", "F2")

    y = 615
    for label, value in rows:
        # Keep each row compact and safe for long values.
        label = str(label)
        value = str(value)
        if len(value) > 64:
            value = value[:61] + "..."
        fill_rect(75, y-5, 445, 24, 0.93, 0.96, 0.99)
        txt(88, y+3, 9, f"{label}:", "F2")
        txt(235, y+3, 9, value)
        y -= 30
        if y < 300:
            break
    ...
    # Status box.
    fill_rect(75, y-8, 445, 32, 0.08, 0.42, 0.18)
    txt(145, y+3, 11, f"Status: {status}", "F2")
    y -= 55
    line(75, y, 520, y, 1)
    txt(172, y-25, 8, "GLOBEXOMART - Premium Methods, Products & Guidance")
    txt(205, y-40, 8, "Support / Bot: @globexomartbot")
    txt(155, y-58, 8, "Thank you for trusting GLOBEXOMART. Keep this invoice for your records.")
    ...
    objects.append(b"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 5 0 R /F2 6 0 R >> >> /Contents 4 0 R >>")
...
def _send_globexomart_invoice(uid, invoice_type, invoice_id, rows, status="Paid / Delivered Successfully"):
    """Generate and send an official GLOBEXOMART invoice to a user."""
    try:
        ...
        raw_bot.send_message(uid, "🧾 Generating your official GLOBEXOMART invoice...")
        raw_bot.send_document(
            uid,
            pdf,
            caption=f"🧾 GLOBEXOMART {invoice_type} Invoice\nInvoice: #{invoice_id}\n\nThank you for trusting GLOBEXOMART. 💎",
        )
        return True
    except Exception as exc:
