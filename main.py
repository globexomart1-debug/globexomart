...
def gift_admin_menu(m):
    raw_bot.send_message(
        m.chat.id,
        "🎁 GIFT LINK MANAGER\n\nImport a TXT file or paste links in bulk. Mobile and PC links are stored in separate pools. A link is retired after five different users report it as not working.",
...
def gift_admin_cb(c):
    ...
    if action=="import" and len(parts)>2:
        ...
        msg=raw_bot.send_message(
            c.from_user.id,
            f"📥 IMPORT {_gift_kind_label(kind).upper()} LINKS\n\nSend a .txt file or paste all URLs in one message. Links can be one per line or mixed with labels such as 'Phone Login URL:'.",
        )
