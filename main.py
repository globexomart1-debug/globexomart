...
import os, time, random, string, threading, hashlib, hmac, json, csv, io, zipfile, traceback, logging, re, unicodedata, math
...
# Personalized referral links/percentages and the Gift login-link pools.
referral_custom_links_col = db["referral_custom_links"]
gift_links_col = db["gift_login_links"]
gift_link_reports_col = db["gift_link_reports"]
gift_link_deliveries_col = db["gift_link_deliveries"]
gift_access_purchases_col = db["gift_access_purchases"]
...
def ensure_indexes():
    index_jobs = [
        ...
        (preorders_col, [("status", 1), ("created_at", -1)], {}),
        (preorders_col, [("user_id", 1), ("created_at", -1)], {}),
        ...
        (referral_custom_links_col, "code", {"unique": True}),
        (referral_custom_links_col, [("user_id", 1), ("active", 1), ("created_at", -1)], {}),
        (gift_links_col, [("kind", 1), ("url", 1)], {"unique": True}),
        (gift_links_col, [("kind", 1), ("status", 1), ("report_count", 1), ("delivery_count", 1)], {}),
        (gift_link_reports_col, [("link_id", 1), ("user_id", 1)], {"unique": True}),
        (gift_link_deliveries_col, [("user_id", 1), ("kind", 1), ("created_at", -1)], {}),
        (gift_link_deliveries_col, [("link_id", 1), ("user_id", 1)], {"unique": True}),
        (gift_access_purchases_col, "user_id", {"unique": True}),
...
# Keep the currently open reply-keyboard/submenu after a user/admin answers a
# next-step prompt.  Many legacy completion handlers explicitly attach
# main_menu()/admin_menu(), which made Telegram jump back automatically.
_FLOW_REPLY_CONTEXT = threading.local()
_FLOW_REPLY_CONTEXT.active = False
_ORIGINAL_BOT_SEND_MESSAGE = bot.send_message
_ORIGINAL_RAW_SEND_MESSAGE = raw_bot.send_message

def _is_root_reply_keyboard(markup):
    if markup is None:
        return False
    # Inline keyboards must never be touched. Root/submenu keyboards are
    # ReplyKeyboardMarkup objects. During a next-step completion we preserve
    # the keyboard already visible on the client by omitting the replacement.
    return isinstance(markup, ReplyKeyboardMarkup)

def _flow_safe_send(original):
    def wrapped(*args, **kwargs):
        if getattr(_FLOW_REPLY_CONTEXT, "active", False) and _is_root_reply_keyboard(kwargs.get("reply_markup")):
            kwargs = dict(kwargs)
            kwargs.pop("reply_markup", None)
        return original(*args, **kwargs)
    return wrapped

bot.send_message = _flow_safe_send(_ORIGINAL_BOT_SEND_MESSAGE)
raw_bot.send_message = _flow_safe_send(_ORIGINAL_RAW_SEND_MESSAGE)
...
def _register_cancellable_next_step(message, callback, *args, **kwargs):
    ...
    def guarded_callback(next_message, *cb_args, **cb_kwargs):
        if _is_cancel_input(next_message):
            ...
            return None
        # Mark next-step execution so completion replies do not replace the
        # user's/admin's current submenu keyboard with the root menu.
        _FLOW_REPLY_CONTEXT.active = True
        try:
...
def get_config():
    ...
    if not cfg:
        cfg = {
            ...
            "gift_button_name": "🎁 Gift",
            "gift_bad_report_limit": 5,
            "gift_price_currency": "points",
            "gift_price_amount": 0,
...
MAIN_MENU_ROWS = [
    ...
    ("❓ FAQ", "🌐 Language"),
    ("🔎 Scanners",),
    ("📍 Order Tracker", "📦 Delivery Center"),
    ("💎 Renewal Center", "🛡 Warranty & Replacement"),
    ("🔔 Expiry Center", "🎁 Reward Center"),
    ("📣 Announcements", "💰 Earnings Dashboard"),
    ("🧰 More",),
]
...
def _default_main_menu_custom_rows():
    """Build an initial editable row layout from the current saved order.

    Legacy main_menu_columns is used only once as a migration/default so existing
    installations keep roughly the same appearance until admin edits rows.
    """
    order = get_main_menu_order()
    try:
        legacy_cols = max(1, min(3, int(get_cached_config().get("main_menu_columns", 2) or 2)))
    except Exception:
        legacy_cols = 2
    return [order[i:i + legacy_cols] for i in range(0, len(order), legacy_cols)]

def get_main_menu_custom_rows():
    cfg = get_cached_config()
    saved = cfg.get("main_menu_custom_rows")
    if not isinstance(saved, list):
        return _default_main_menu_custom_rows()
    rows = []
    used = set()
    for row in saved:
        ...
        clean = []
        for button in row:
            if button in MAIN_MENU_BUTTONS and button not in used and len(clean) < 3:
                clean.append(button)
                used.add(button)
        if clean:
            rows.append(clean)
    # Newly introduced buttons are never lost. Append them as their own rows so
    # admin can later place them wherever desired.
    for button in get_main_menu_order():
        if button not in used:
            rows.append([button])
            used.add(button)
    return rows

def save_main_menu_custom_rows(rows):
    clean_rows = []
    used = set()
    for row in rows or []:
        if not isinstance(row, list):
    ...
    # Keep every known button available; unassigned buttons are appended as
    # single-button rows rather than disappearing from the bot.
    for button in get_main_menu_order():
        if button not in used:
            clean_rows.append([button])
            used.add(button)
    set_config("main_menu_custom_rows", clean_rows)
    # Maintain the legacy flat order for features that still rely on it.
    save_main_menu_order([b for row in clean_rows for b in row])
    return clean_rows

# Buttons placed inside the user-facing 🧰 More submenu instead of the main keyboard.
# Admin can change this list at any time from Arrange Main Buttons -> Manage More.
DEFAULT_MORE_BUTTONS = [
    "🔥 Deals", "🏆 Ref Leaderboard", "❤️ Favorites", "🔍 Search All",
    "🎫 Support", "🛒 Cart", "🧾 My Orders", "🔔 Watchlist",
    "🎖 Loyalty", "❓ FAQ", "🌐 Language", "🔎 Scanners",
    "🔥 Trending", "🔔 Preferences", "🧩 Bundles", "🧭 My Hub",
    "🔎 Filter Search",
    # V34 user tools start inside More by default. Admin can pull any of them
    # back to the main menu from Arrange Main Buttons -> Manage More Menu.
    "📍 Order Tracker", "📦 Delivery Center", "💎 Renewal Center",
    "🛡 Warranty & Replacement", "🔔 Expiry Center", "🎁 Reward Center",
    "📣 Announcements", "💰 Earnings Dashboard",
]

def get_more_menu_buttons():
    cfg = get_cached_config()
    saved = cfg.get("more_menu_buttons")
    if not isinstance(saved, list):
        saved = list(DEFAULT_MORE_BUTTONS)
    valid = []
    seen = set()
    for button in saved:
        if button in MAIN_MENU_BUTTONS and button != "🧰 More" and button not in seen:
            valid.append(button)
            seen.add(button)
    return valid

def save_more_menu_buttons(buttons):
    clean = []
    seen = set()
    for button in buttons:
        if button in MAIN_MENU_BUTTONS and button != "🧰 More" and button not in seen:
            clean.append(button)
            seen.add(button)
    set_config("more_menu_buttons", clean)
    return clean

def _button_visible_for_user(button, uid, hidden=None):
    hidden = hidden if hidden is not None else get_hidden_main_buttons()
    if button in hidden:
        return False
...
def more_menu(uid):
    # The main menu now has per-row layouts. Keep the More submenu compact at
    # two buttons per row; its membership is still fully admin-managed.
    columns = 2
    kb = ReplyKeyboardMarkup(resize_keyboard=True, row_width=columns)
    hidden = get_hidden_main_buttons()
    buttons = [b for b in get_more_menu_buttons() if _button_visible_for_user(b, uid, hidden)]
    for i in range(0, len(buttons), columns):
        kb.row(*buttons[i:i + columns])
    # Keep V28's extra learning/affiliate tools available without cluttering the main menu.
    kb.row("✨ More Tools")
    kb.row("⬅️ Main Menu")
    if is_admin(uid):
...
def main_menu(uid):
    ...
    hidden = get_hidden_main_buttons()
    more_buttons = set(get_more_menu_buttons())

    # Each row is independently configured by admin and may contain 1, 2 or 3
    # buttons. Telegram automatically sizes the buttons within that row.
    placed = set()
    for configured_row in get_main_menu_custom_rows():
        visible_row = []
        for button in configured_row:
            if button in placed:
                continue
            if button != "🧰 More" and button in more_buttons:
                continue
            if not _button_visible_for_user(button, uid, hidden):
                continue
            visible_row.append(button)
            placed.add(button)
        if visible_row:
            kb.row(*visible_row[:3])

    # Custom buttons are kept compatible with the existing system. They are
    # displayed two per row because they are not part of MAIN_MENU_BUTTONS.
...
def finalize_pending_referral(uid, telegram_user=None):
    ...
    row = users_col.find_one(
        {"_id": uid_str},
        {"pending_ref": 1, "pending_ref_percent": 1, "pending_ref_link_code": 1, "ref": 1, "username": 1},
    ) or {}
    ...
    if str(ref_id) == uid_str or not str(ref_id).isdigit():
        users_col.update_one(
            {"_id": uid_str},
            {"$unset": {"pending_ref": "", "pending_ref_percent": "", "pending_ref_link_code": ""}},
        )
        return False
    ...
    claim_values = {"ref": str(ref_id)}
    try:
        if row.get("pending_ref_percent") is not None:
            claim_values["referral_percent_locked"] = max(0.0, min(100.0, float(row["pending_ref_percent"])))
    except Exception:
        pass
    if row.get("pending_ref_link_code"):
        claim_values["referral_link_code"] = str(row["pending_ref_link_code"])[:32]
    ...
    if row.get("pending_ref_link_code"):
        referral_custom_links_col.update_one(
            {"code": str(row["pending_ref_link_code"]), "user_id": int(ref_id)},
            {"$inc": {"qualified_referrals": 1}, "$set": {"last_qualified_at": time.time()}},
        )
...
def start_cmd(m):
    ...
    if len(args) > 1:
        ...
        elif payload.startswith("rfl_"):
            code = payload[4:].strip().upper()
            custom_link = referral_custom_links_col.find_one({"code": code, "active": {"$ne": False}})
            if custom_link and int(custom_link.get("user_id", 0) or 0) != int(uid) and not user.data.get("ref"):
                ref_id = str(int(custom_link["user_id"]))
                if users_col.find_one({"_id": ref_id}, {"_id": 1}):
                    users_col.update_one(
                        {"_id": str(uid), "$or": [{"ref": None}, {"ref": {"$exists": False}}]},
                        {"$set": {
                            "pending_ref": ref_id,
                            "pending_ref_percent": max(0.0, min(100.0, float(custom_link.get("percent", 0) or 0))),
                            "pending_ref_link_code": code,
                            "acquisition_source": "custom_referral:" + code,
                        }},
                    )
                    referral_custom_links_col.update_one(
                        {"_id": custom_link["_id"]},
                        {"$inc": {"start_count": 1}, "$set": {"last_started_at": time.time()}},
                    )
...
def _earn_plan_lines(uid=None):
    cfg = get_cached_config()
    if uid is not None and "_effective_referral_percent" in globals():
        special_link = referral_custom_links_col.find_one(
            {"user_id": int(uid), "active": {"$ne": False}},
            sort=[("created_at", -1)],
        )
        pct = _effective_referral_percent(referrer_id=uid, special_link=special_link)
    else:
        pct = max(0.0, min(100.0, float(cfg.get("referral_commission_percent", 15) or 0)))
...
def send_earn_menu(uid):
    cfg = get_cached_config()
    pct, plan_lines = _earn_plan_lines(uid)
...
def admin_menu():
    ...
    kb.row("⏳ Pending Orders", "📦 Pending Preorders")
...
def _shop_is_available(product):
    if product.get("preorder_enabled"):
        return bool(product.get("active", True)) and not bool(product.get("force_out_of_stock", False))
...
def _shop_status(product):
    if not product.get("active", True):
        return "HIDDEN"
    if product.get("preorder_enabled"):
        return "PREORDER OPEN" if not product.get("force_out_of_stock", False) else "PREORDER CLOSED"
...
def _shop_card(product, admin=False):
    ...
    if product.get("preorder_enabled"):
        text += f"📦 Type: PREORDER\n🗓 Expected: {product.get('preorder_eta') or 'To be announced'}\n"
    if admin:
...
def shop_view_cb(c):
    ...
    if _shop_is_available(product):
        if product.get("preorder_enabled") and product.get("kind")=="paid":
            kb.add(InlineKeyboardButton("📦 Preorder Now — Pay in Advance", callback_data=f"preorderask|{product['_id']}"))
        else:
...
def _shop_start_admin_upload(uid, kind, preorder=False):
    _product_admin_state[uid]={"mode":"create","kind":kind,"preorder":bool(preorder),"step":"name","data":{}}
    label="PREORDER PRODUCT" if preorder else ("FREE PRODUCT" if kind=="free" else "PAID PRODUCT")
...
def _shop_save_new_product(uid,state):
    ...
    doc={
        ...
        "preorder_enabled":bool(state.get("preorder",False)),
        "preorder_eta":d.get("preorder_eta") or "",
...
def shop_admin_wizard_message(m):
    ...
    try:
        ...
        if step=="warranty":
            d["warranty"]=text[:160]
            if state.get("preorder"):
                state["step"]="preorder_eta"
                return raw_bot.send_message(uid,"Step 6/7 — Send expected delivery/release time for this preorder. Example: 7-10 Days, 15 September, Q4 2026, or /skip.")
            state["step"]="stock"
        ...
        if step=="preorder_eta":
            d["preorder_eta"]="" if text.lower()=="/skip" else text[:160]
            d["stock_raw"]=""
            state["step"]="delivery_note"
            return raw_bot.send_message(uid,"Step 7/7 — Send preorder/delivery instructions shown after payment, or /skip.")
        if step=="stock":
...
def _shop_admin_product_keyboard(product):
    ...
    if product.get("kind")=="paid":
        kb.add(InlineKeyboardButton("📦 Toggle Preorder",callback_data=f"shopadm_preorder|{pid}"),InlineKeyboardButton("🗓 Preorder ETA",callback_data=f"shopadm_edit|preorder_eta|{pid}"))
...
def _shop_send_manager(uid):
    ...
    kb.add(InlineKeyboardButton("📦 Add Preorder Product",callback_data="shopadm_new|preorder"))
    kb.add(InlineKeyboardButton("🧾 Product Orders",callback_data="shopadm_orders"),InlineKeyboardButton("📦 Pending Preorders",callback_data="preorder_admin_list"))
    kb.add(InlineKeyboardButton("📊 Product Stats",callback_data="shopadm_stats"))
    for p in rows:
...
def shop_admin_new_cb(c):
    ...
    kind=c.data.split("|",1)[1]
    if kind=="preorder":
        _shop_start_admin_upload(c.from_user.id,"paid",preorder=True)
    else:
...
def shop_admin_edit_start(c):
    ...
    prompts={
        ...
        "preorder_eta":"Send expected preorder delivery/release time (send clear to remove):",
    }
...
# =========================
# ↕️ MAIN MENU ROW BUILDER
# =========================
def _row_builder_text(selected_row=None):
    rows = get_main_menu_custom_rows()
    lines = [
        "🧱 MAIN MENU ROW BUILDER",
        "",
        "Each row can contain 1, 2 or 3 buttons independently.",
        "Tap a row to choose exactly which buttons belong in it. Telegram automatically fits their widths.",
        "",
    ]
    for i, row in enumerate(rows):
        marker = "👉 " if selected_row == i else ""
        lines.append(f"{marker}Row {i+1}: " + "  |  ".join(row))
    lines.append("\nButtons moved into 🧰 More remain excluded from the main keyboard.")
    return "\n".join(lines)


def _row_builder_markup(selected_row=None):
    rows = get_main_menu_custom_rows()
    kb = InlineKeyboardMarkup(row_width=1)
    for i, row in enumerate(rows):
        preview = " | ".join(row)
        if len(preview) > 45:
            preview = preview[:42] + "..."
        kb.add(InlineKeyboardButton(f"Row {i+1} • {preview}", callback_data=f"mmrow|open|{i}"))
    if selected_row is not None and 0 <= selected_row < len(rows):
        kb.row(
            InlineKeyboardButton("⬆️ Row Up", callback_data=f"mmrow|move|{selected_row}|up"),
            InlineKeyboardButton("⬇️ Row Down", callback_data=f"mmrow|move|{selected_row}|down"),
        )
    kb.row(InlineKeyboardButton("➕ Add New Row", callback_data="mmrow|add"))
    kb.row(InlineKeyboardButton("🧰 Manage More Menu", callback_data="mmord|more|0"))
    kb.row(
        InlineKeyboardButton("🔄 Reset Rows", callback_data="mmrow|reset"),
        InlineKeyboardButton("✅ Done", callback_data="mmrow|done"),
    )
    return kb


def _row_edit_text(row_index):
    rows = get_main_menu_custom_rows()
    if row_index < 0 or row_index >= len(rows):
        return "Row not found."
    row = rows[row_index]
    return (
        f"🧱 EDIT ROW {row_index+1}\n\n"
        f"Current: {' | '.join(row) if row else 'Empty'}\n\n"
        "Select 1, 2 or 3 buttons for this row.\n"
        "A button can belong to only one row. Selecting it here automatically moves it from its previous row."
    )


def _row_edit_markup(row_index, page=0):
    rows = get_main_menu_custom_rows()
    if row_index < 0 or row_index >= len(rows):
        return InlineKeyboardMarkup()
    current = list(rows[row_index])
    candidates = [b for b in get_main_menu_order()]
    per_page = 10
    total_pages = max(1, (len(candidates) + per_page - 1)//per_page)
    page = max(0, min(int(page or 0), total_pages-1))
    start = page*per_page
    kb = InlineKeyboardMarkup(row_width=1)
    for idx in range(start, min(len(candidates), start+per_page)):
        b = candidates[idx]
        mark = "✅" if b in current else "➕"
        kb.add(InlineKeyboardButton(f"{mark} {b}", callback_data=f"mmrow|toggle|{row_index}|{idx}|{page}"))
    nav=[]
    if page>0: nav.append(InlineKeyboardButton("⬅️ Previous", callback_data=f"mmrow|editpage|{row_index}|{page-1}"))
    if page+1<total_pages: nav.append(InlineKeyboardButton("Next ➡️", callback_data=f"mmrow|editpage|{row_index}|{page+1}"))
    if nav: kb.row(*nav)
    kb.row(
        InlineKeyboardButton("🧹 Clear Row", callback_data=f"mmrow|clear|{row_index}"),
        InlineKeyboardButton("🗑 Delete Row", callback_data=f"mmrow|delete|{row_index}"),
    )
    kb.row(InlineKeyboardButton("⬅️ Back to Rows", callback_data="mmrow|back"))
    return kb


def _more_manager_markup(page=0):
    candidates = [b for b in get_main_menu_order() if b != "🧰 More"]
    selected = set(get_more_menu_buttons())
    per_page = 10
    total_pages = max(1, (len(candidates) + per_page - 1) // per_page)
    page = max(0, min(int(page or 0), total_pages - 1))
    start = page * per_page
    ...
    for idx in range(start, min(len(candidates), start + per_page)):
        ...
        kb.add(InlineKeyboardButton(f"{mark} {b}", callback_data=f"mmord|moretoggle|{idx}|{page}"))
    nav=[]
    if page>0: nav.append(InlineKeyboardButton("⬅️ Previous",callback_data=f"mmord|more|{page-1}"))
    if page+1<total_pages: nav.append(InlineKeyboardButton("Next ➡️",callback_data=f"mmord|more|{page+1}"))
    if nav: kb.row(*nav)
    kb.row(
        InlineKeyboardButton("📦 Use Suggested Set", callback_data="mmord|moredefault"),
        InlineKeyboardButton("🧹 Empty More", callback_data="mmord|moreclear"),
    )
    kb.row(InlineKeyboardButton("⬅️ Back to Arrange",callback_data="mmord|moreback"))
    return kb


def _more_manager_text():
    selected=get_more_menu_buttons()
    return (
        "🧰 MANAGE MORE MENU\n\n"
        "Tap any existing main button to move it into or out of 🧰 More. "
        "Buttons inside More are automatically removed from the main keyboard, but their original functions stay unchanged.\n\n"
        f"Currently inside More: {len(selected)}"
    )
...
def arrange_main_buttons_menu(m):
    raw_bot.send_message(m.from_user.id, _row_builder_text(), reply_markup=_row_builder_markup())


@bot.callback_query_handler(func=lambda c: str(c.data or "").startswith("mmrow|"))
def main_row_builder_callback(c):
    if not is_admin(c.from_user.id):
        return bot.answer_callback_query(c.id, "Admins only", True)
    try:
        parts = c.data.split("|")
        ...
        if action == "done":
            ...
            return raw_bot.send_message(c.from_user.id, "✅ Main menu row layout saved.", reply_markup=admin_menu())

        if action == "back":
            raw_bot.edit_message_text(_row_builder_text(), c.message.chat.id, c.message.message_id, reply_markup=_row_builder_markup())
            return bot.answer_callback_query(c.id)

        if action == "reset":
            set_config("main_menu_custom_rows", _default_main_menu_custom_rows())
            raw_bot.edit_message_text(_row_builder_text(), c.message.chat.id, c.message.message_id, reply_markup=_row_builder_markup())
            return bot.answer_callback_query(c.id, "Default rows restored")

        if action == "add":
            # Add a temporary empty row by moving the last available button into it.
            used = {b for row in rows for b in row}
            candidates = [b for b in get_main_menu_order() if b not in used]
            if candidates:
                rows.append([candidates[0]])
            else:
                # All buttons already belong somewhere; start by moving the last
                # button from the last row into a new row when possible.
                donor = next((i for i in range(len(rows)-1, -1, -1) if len(rows[i]) > 1), None)
                if donor is None:
                    return bot.answer_callback_query(c.id, "Every button is already in a single-button row. Edit an existing row instead.", True)
                rows.append([rows[donor].pop()])
            save_main_menu_custom_rows(rows)
            idx = len(get_main_menu_custom_rows())-1
            raw_bot.edit_message_text(_row_edit_text(idx), c.message.chat.id, c.message.message_id, reply_markup=_row_edit_markup(idx,0))
            return bot.answer_callback_query(c.id, "New row created")

        if action == "open":
            ri = int(parts[2])
            raw_bot.edit_message_text(_row_edit_text(ri), c.message.chat.id, c.message.message_id, reply_markup=_row_edit_markup(ri,0))
            return bot.answer_callback_query(c.id)

        if action == "editpage":
            ri, page = int(parts[2]), int(parts[3])
            raw_bot.edit_message_text(_row_edit_text(ri), c.message.chat.id, c.message.message_id, reply_markup=_row_edit_markup(ri,page))
            return bot.answer_callback_query(c.id)

        if action == "toggle":
            ri, ci, page = int(parts[2]), int(parts[3]), int(parts[4])
            candidates = get_main_menu_order()
            if ri < 0 or ri >= len(rows) or ci < 0 or ci >= len(candidates):
                return bot.answer_callback_query(c.id, "Row/button not found", True)
            b = candidates[ci]
            if b in rows[ri]:
                if len(rows[ri]) <= 1:
                    return bot.answer_callback_query(c.id, "A row must keep at least 1 button. Choose another button first or delete the row.", True)
                rows[ri] = [x for x in rows[ri] if x != b]
                msg = "Removed from row"
            else:
                if len(rows[ri]) >= 3:
                    return bot.answer_callback_query(c.id, "Maximum 3 buttons in one row", True)
                # Remove from any old row, then add here.
                new_rows=[]
                for j,row in enumerate(rows):
                    if j == ri:
                        new_rows.append(list(row))
                        continue
                    cleaned=[x for x in row if x != b]
                    if cleaned:
                ...
                rows=new_rows
                rows[ri].append(b)
                msg = "Moved into this row"
            save_main_menu_custom_rows(rows)
            rows=get_main_menu_custom_rows()
            ri=max(0,min(ri,len(rows)-1))
            raw_bot.edit_message_text(_row_edit_text(ri), c.message.chat.id, c.message.message_id, reply_markup=_row_edit_markup(ri,page))
            return bot.answer_callback_query(c.id, msg)

        if action == "clear":
            ri=int(parts[2])
            if ri<0 or ri>=len(rows): return bot.answer_callback_query(c.id,"Row not found",True)
            if len(rows)<=1: return bot.answer_callback_query(c.id,"Cannot clear the only row",True)
            rows.pop(ri)
            save_main_menu_custom_rows(rows)
            raw_bot.edit_message_text(_row_builder_text(), c.message.chat.id, c.message.message_id, reply_markup=_row_builder_markup())
            return bot.answer_callback_query(c.id,"Row removed; its buttons were kept available")

        if action == "delete":
            ri=int(parts[2])
            if ri<0 or ri>=len(rows): return bot.answer_callback_query(c.id,"Row not found",True)
            if len(rows)<=1: return bot.answer_callback_query(c.id,"Cannot delete the only row",True)
            rows.pop(ri)
            save_main_menu_custom_rows(rows)
            raw_bot.edit_message_text(_row_builder_text(), c.message.chat.id, c.message.message_id, reply_markup=_row_builder_markup())
            return bot.answer_callback_query(c.id,"Row deleted; buttons were preserved")

        if action == "move":
            ri=int(parts[2]); direction=parts[3]
            if ri<0 or ri>=len(rows): return bot.answer_callback_query(c.id,"Row not found",True)
            new=ri
            if direction=="up" and ri>0:
                rows[ri-1],rows[ri]=rows[ri],rows[ri-1]; new=ri-1
            elif direction=="down" and ri<len(rows)-1:
                rows[ri+1],rows[ri]=rows[ri],rows[ri+1]; new=ri+1
            save_main_menu_custom_rows(rows)
            raw_bot.edit_message_text(_row_builder_text(new), c.message.chat.id, c.message.message_id, reply_markup=_row_builder_markup(new))
            return bot.answer_callback_query(c.id,"Row position updated")

        bot.answer_callback_query(c.id)
    except Exception as exc:
        log_event("main_row_builder_error", c.from_user.id, details={"error": str(exc)}, level="error")
        bot.answer_callback_query(c.id, "Could not update row layout", True)
...
def main_button_order_callback(c):
    ...
    try:
        ...
        if action == "more":
            ...
            raw_bot.edit_message_text(_more_manager_text(), c.message.chat.id, c.message.message_id, reply_markup=_more_manager_markup(page))
            return bot.answer_callback_query(c.id)
        if action == "moretoggle":
            idx = int(parts[2]); page = int(parts[3])
            candidates = [b for b in get_main_menu_order() if b != "🧰 More"]
            if idx < 0 or idx >= len(candidates): return bot.answer_callback_query(c.id, "Button not found", True)
            button = candidates[idx]
            current = get_more_menu_buttons()
            if button in current:
                current = [b for b in current if b != button]; msg = "Removed from More"
            else:
                current.append(button); msg = "Moved into More"
            save_more_menu_buttons(current)
            raw_bot.edit_message_text(_more_manager_text(), c.message.chat.id, c.message.message_id, reply_markup=_more_manager_markup(page))
            return bot.answer_callback_query(c.id, msg)
        if action == "moredefault":
            save_more_menu_buttons(DEFAULT_MORE_BUTTONS)
            raw_bot.edit_message_text(_more_manager_text(), c.message.chat.id, c.message.message_id, reply_markup=_more_manager_markup(0))
            return bot.answer_callback_query(c.id, "Suggested More menu restored")
        if action == "moreclear":
            save_more_menu_buttons([])
            raw_bot.edit_message_text(_more_manager_text(), c.message.chat.id, c.message.message_id, reply_markup=_more_manager_markup(0))
            return bot.answer_callback_query(c.id, "More menu cleared")
        if action == "moreback":
            raw_bot.edit_message_text(_row_builder_text(), c.message.chat.id, c.message.message_id, reply_markup=_row_builder_markup())
            return bot.answer_callback_query(c.id)
        bot.answer_callback_query(c.id)
    except Exception as exc:
        log_event("main_button_more_error", c.from_user.id, details={"error": str(exc)}, level="error")
        bot.answer_callback_query(c.id, "Could not update More menu", True)
...
def review_manual_subscription_payment_cb(c):
    ...
    try:
        ...
        proof_record = dict(pay)
        proof_record["expires_at"] = sub.get("expires_at")
        _publish_proof("VIP", proof_record, screenshot_chat_id=pay.get("screenshot_chat_id"), screenshot_message_id=pay.get("screenshot_message_id"))
        ...
        try:
            review_kb = InlineKeyboardMarkup(row_width=1)
            review_kb.add(InlineKeyboardButton("⭐ Review Your VIP Experience", callback_data=f"ratevip|{plan_code}"))
            raw_bot.send_message(uid, msg, reply_markup=review_kb, disable_web_page_preview=True)
        except Exception:
            pass
    except Exception as exc:
        bot.answer_callback_query(c.id, "Review failed", True)
...
def admin_subadd_cmd(m):
    ...
    try:
        ...
        try:
            review_kb = InlineKeyboardMarkup(row_width=1)
            review_kb.add(InlineKeyboardButton("⭐ Review Your VIP Experience", callback_data=f"ratevip|{plan}"))
            raw_bot.send_message(uid, user_text, reply_markup=review_kb, disable_web_page_preview=True)
        except Exception:
            pass
    except Exception as exc:
        admin_error(m.from_user.id, exc)
...
def _clean_referral_percent(value, default=0.0):
    try:
...
def _effective_referral_percent(referrer_id=None, buyer=None, special_link=None):
    """Resolve rate: locked special-link rate, user override, then global rate."""
    global_rate = _clean_referral_percent(get_cached_config().get("referral_commission_percent", 15), 15)
    if buyer and buyer.get("referral_percent_locked") is not None:
        return _clean_referral_percent(buyer.get("referral_percent_locked"), global_rate)
    if special_link and special_link.get("percent") is not None:
        return _clean_referral_percent(special_link.get("percent"), global_rate)
    if referrer_id is not None:
        referrer = users_col.find_one(
            {"_id": str(int(referrer_id))},
            {"referral_commission_percent_override": 1},
        ) or {}
        if referrer.get("referral_commission_percent_override") is not None:
            return _clean_referral_percent(referrer.get("referral_commission_percent_override"), global_rate)
    return global_rate
...
def _process_referral_vip_sale(buyer_id, amount, sub):
    ...
    if not ref or not str(ref).isdigit() or str(ref)==str(buyer_id):
        return
    referrer_id=int(ref)
    pct=_effective_referral_percent(referrer_id=referrer_id,buyer=buyer)
...
def send_referral_card(uid):
    ...
    link=(f"https://t.me/{username}?start=rfl_{special_link['code']}" if special_link
          else f"https://t.me/{username}?start={uid}")
    ...
    text=(f"🎁 GLOBEXOMART REFER & EARN\n━━━━━━━━━━━━━━━━━━━━\n\n"
          f"Turn your network into earnings. Share your personal link and earn {pct:g}% every time one of your referred users buys a VIP plan.\n\n"
...
def referral_settings_menu(m):
    ...
    kb.add(InlineKeyboardButton("👤 Set User Referral %",callback_data="refcustom|set"))
    kb.add(InlineKeyboardButton("🔗 Generate Special Referral Link",callback_data="refcustom|generate"))
    kb.add(InlineKeyboardButton("📋 Special Links & Rates",callback_data="refcustom|list"))
...
def referral_settings_cb(c):
    ...
    prompts={"commission":"Send default referral commission percent (0-100):","min":"Send minimum referral withdrawal in USDT:","bonus":"Send bonus as target,amount  Example: 10,10"}
...
def referral_setting_save(m,action):
    try:
        if action=="commission":
            v=float(m.text); 
            if not 0<=v<=100: raise ValueError("Commission must be 0-100")
...
def _find_referral_user(value):
    value=str(value or "").strip()
    if value.startswith("@"): value=value[1:]
    if value.isdigit():
        row=users_col.find_one({"_id":str(int(value))})
    else:
        row=users_col.find_one({"username":{"$regex":"^"+re.escape(value)+"$","$options":"i"}})
    if not row:
        raise ValueError("User not found. Ask the user to start the bot first, then use their Telegram ID or @username.")
    return row


def _new_referral_link_code():
    alphabet=string.ascii_uppercase+string.digits
    for _ in range(50):
        code="".join(random.choice(alphabet) for _ in range(10))
        if not referral_custom_links_col.find_one({"code":code},{"_id":1}):
            return code
    raise RuntimeError("Could not create a unique referral code. Try again.")


@bot.callback_query_handler(func=lambda c:str(c.data or "").startswith("refcustom|"))
def referral_custom_admin_cb(c):
    if not is_admin(c.from_user.id):
        return bot.answer_callback_query(c.id,"Admin only",True)
    parts=c.data.split("|")
    ...
    if action=="set":
        msg=raw_bot.send_message(
            c.from_user.id,
            "👤 SET USER REFERRAL RATE\n\nSend: USER_ID PERCENT\nExample: 123456789 25\n\nTo restore the global rate, send: USER_ID default",
        )
        bot.register_next_step_handler(msg,referral_custom_rate_save)
    elif action=="generate":
        msg=raw_bot.send_message(
            c.from_user.id,
            "🔗 GENERATE SPECIAL REFERRAL LINK\n\nSend: USER_ID PERCENT optional label\nExample: 123456789 30 KOL Partner\n\nThe percentage is locked to referrals who join through this exact link.",
        )
        bot.register_next_step_handler(msg,referral_custom_link_generate)
    elif action=="list":
        referral_custom_links_list(c.from_user.id)
    elif action=="toggle" and len(parts)>2:
        try:
            oid=ObjectId(parts[2]); row=referral_custom_links_col.find_one({"_id":oid})
            if not row: raise ValueError("Link not found")
            active=not bool(row.get("active",True))
            referral_custom_links_col.update_one(
                {"_id":oid},
                {"$set":{"active":active,"updated_at":time.time(),"updated_by":int(c.from_user.id)}},
            )
            bot.answer_callback_query(c.id,"Link activated" if active else "Link deactivated",True)
            return referral_custom_links_list(c.from_user.id)
        except Exception as exc:
            return bot.answer_callback_query(c.id,str(exc)[:180],True)
    bot.answer_callback_query(c.id)


def referral_custom_rate_save(m):
    try:
        parts=(m.text or "").replace(","," ").split()
        if len(parts)!=2: raise ValueError("Use: USER_ID PERCENT or USER_ID default")
        row=_find_referral_user(parts[0]); uid=int(row["_id"])
        if parts[1].lower() in {"default","remove","reset"}:
            users_col.update_one({"_id":str(uid)},{"$unset":{"referral_commission_percent_override":""}})
            rate=_effective_referral_percent(referrer_id=uid)
            text=f"User {uid} now uses the global referral rate: {rate:g}%."
        else:
            rate=float(parts[1])
            if not 0<=rate<=100: raise ValueError("Percentage must be between 0 and 100")
            users_col.update_one(
                {"_id":str(uid)},
                {"$set":{"referral_commission_percent_override":rate,"referral_rate_updated_at":time.time(),"referral_rate_updated_by":int(m.from_user.id)}},
            )
            text=f"User {uid} referral rate set to {rate:g}%. Their normal personal referral link now uses this rate."
        try: raw_bot.send_message(uid,f"🤝 Your GLOBEXOMART referral commission rate is now {rate:g}%.")
        except Exception: pass
        admin_success(m.from_user.id,text)
    except Exception as exc:
        admin_error(m.from_user.id,exc)


def referral_custom_link_generate(m):
    try:
        parts=(m.text or "").strip().split(None,2)
        if len(parts)<2: raise ValueError("Use: USER_ID PERCENT optional label")
        row=_find_referral_user(parts[0]); uid=int(row["_id"]); rate=float(parts[1])
        if not 0<=rate<=100: raise ValueError("Percentage must be between 0 and 100")
        label=(parts[2].strip() if len(parts)>2 else "Special referral link")[:80]
        code=_new_referral_link_code()
        referral_custom_links_col.insert_one({
            "code":code,"user_id":uid,"percent":rate,"label":label,"active":True,
            "start_count":0,"created_at":time.time(),"created_by":int(m.from_user.id),
        })
        link=f"https://t.me/{bot.get_me().username}?start=rfl_{code}"
        text=f"🔗 SPECIAL REFERRAL LINK CREATED\n\nUser: {uid}\nRate: {rate:g}%\nLabel: {label}\n\n{link}"
        try:
            raw_bot.send_message(uid,f"🔗 YOUR SPECIAL GLOBEXOMART REFERRAL LINK\n\nCommission: {rate:g}%\n{link}",disable_web_page_preview=True)
            text+="\n\n✅ The link was also sent to the user."
        except Exception:
            text+="\n\nℹ️ The user could not be messaged; copy and send the link manually."
        raw_bot.send_message(m.from_user.id,text,reply_markup=admin_menu(),disable_web_page_preview=True)
    except Exception as exc:
        admin_error(m.from_user.id,exc)


def referral_custom_links_list(admin_id):
    rows=list(referral_custom_links_col.find({}).sort("created_at",-1).limit(20))
    if not rows:
        return raw_bot.send_message(admin_id,"📋 No special referral links have been created yet.")
    username=bot.get_me().username; lines=["📋 SPECIAL REFERRAL LINKS",""]
    kb=InlineKeyboardMarkup(row_width=1)
    for row in rows:
        state="ACTIVE" if row.get("active",True) else "OFF"
        link=f"https://t.me/{username}?start=rfl_{row.get('code')}"
        lines.append(f"{state} • User {row.get('user_id')} • {float(row.get('percent',0)):g}% • {row.get('label','')}\nStarts: {int(row.get('start_count',0) or 0)}\n{link}")
        label=("⏸ Deactivate" if row.get("active",True) else "▶️ Activate")+f" {row.get('code')}"
        kb.add(InlineKeyboardButton(label,callback_data=f"refcustom|toggle|{row['_id']}"))
    raw_bot.send_message(admin_id,"\n\n".join(lines)[:3900],reply_markup=kb,disable_web_page_preview=True)
...
def premium_emoji_menu(m):
    ...
    enabled = bool(get_cached_config().get("global_premium_emojis_enabled", True))
    intensity = str(get_cached_config().get("premium_emoji_intensity", "max") or "max").upper()
    kb.add(InlineKeyboardButton("🧪 Test Premium Emoji", callback_data="premiumemoji|test"))
    kb.add(InlineKeyboardButton(("✅ " if enabled else "❌ ") + "Global Premium Emojis", callback_data="premiumemoji|globaltoggle"))
    kb.add(InlineKeyboardButton(f"⚡ Emoji Intensity: {intensity}", callback_data="premiumemoji|intensity"))
    kb.add(InlineKeyboardButton("📥 Import Default 8 Packs", callback_data="premiumemoji|importdefaults"))
    kb.add(InlineKeyboardButton("🧹 Clear Saved Emojis", callback_data="premiumemoji|clear"))
    raw_bot.send_message(m.from_user.id, f"✨ PREMIUM EMOJIS\n\nSaved mappings: {count}\nGlobal use: {'ON' if enabled else 'OFF'}\nIntensity: {intensity}\n\nThe bot can use saved custom emojis globally in generated messages wherever Telegram safely supports custom-emoji entities. MAX mode also adds contextual emoji decoration to plain generated messages. Auto Posts/copies preserve the exact premium emojis already present.\n\nℹ️ Telegram may restrict bots from directly sending custom emojis unless the bot/owner is eligible. Normal Unicode emoji remains as fallback.", reply_markup=kb)
...
def premium_emoji_cb(c):
    if not is_admin(c.from_user.id):
        return bot.answer_callback_query(c.id, "Admin only", True)
    action = c.data.split("|", 1)[1]
    if action == "globaltoggle":
        cur = bool(get_cached_config().get("global_premium_emojis_enabled", True))
        set_config("global_premium_emojis_enabled", not cur)
        bot.answer_callback_query(c.id, "Global premium emojis ON" if not cur else "Global premium emojis OFF", True)
        try: bot.delete_message(c.message.chat.id, c.message.message_id)
        except Exception: pass
        fake = type("obj", (), {"from_user": c.from_user})()
        return premium_emoji_menu(fake)
    if action == "intensity":
        cur = str(get_cached_config().get("premium_emoji_intensity", "max") or "max").lower()
        nxt = {"off":"normal", "normal":"max", "max":"off"}.get(cur, "max")
        set_config("premium_emoji_intensity", nxt)
        bot.answer_callback_query(c.id, f"Emoji intensity: {nxt.upper()}", True)
        try: bot.delete_message(c.message.chat.id, c.message.message_id)
        except Exception: pass
        fake = type("obj", (), {"from_user": c.from_user})()
        return premium_emoji_menu(fake)
    if action == "importdefaults":
        added, failed = _import_default_premium_emoji_packs(force=True)
        bot.answer_callback_query(c.id, f"Imported {added} emoji mappings", True)
        return raw_bot.send_message(c.from_user.id, f"✅ Default premium packs processed.\n✨ New/updated mappings: {added}\n⚠️ Failed packs: {len(failed)}" + (("\n" + ", ".join(failed)) if failed else ""), reply_markup=admin_menu())
    if action == "view":
        mapping = _premium_emoji_map()
...
def _proof_purchase_name(kind, record):
    """Resolve a privacy-safe public item/method/product name for proof posts."""
    kind = str(kind or "").upper()
    # Directly stored names are preferred.
    for key in ("product_name", "method_name", "item_name", "name"):
        value = str(record.get(key) or "").strip()
        if value:
            return value

    # Reseller/API products may store a nested product object.
    nested = record.get("product")
    if isinstance(nested, dict):
        value = str(nested.get("name") or nested.get("title") or "").strip()
        if value:
            return value

    # Manual method/digital product purchases store folder_id.
    folder_id = record.get("folder_id")
    if folder_id:
        try:
            from bson import ObjectId
            folder = folders_col.find_one({"_id": ObjectId(str(folder_id))}, {"name": 1})
            if folder and folder.get("name"):
                return str(folder.get("name")).strip()
        except Exception:
            pass

    # Shop products may only have product_id in older records.
    product_id = record.get("product_id")
    if product_id:
        try:
            from bson import ObjectId
            product = shop_products_col.find_one({"_id": ObjectId(str(product_id))}, {"name": 1})
            if product and product.get("name"):
                return str(product.get("name")).strip()
        except Exception:
            pass

    return "VIP Access" if kind == "VIP" else ("Method" if kind == "METHOD" else "Product")
...
def _publish_proof(kind, record, screenshot_chat_id=None, screenshot_message_id=None):
    ...
    try:
        ...
        if kind == "VIP":
            ...
            lines.append(f"💎 Purchased: {plan_name}")
            if duration_minutes:
                lines.append(f"⏳ Duration: {_format_duration_minutes(duration_minutes)}")

            # Prefer the actual activated subscription expiry passed by approval flow.
            expires_at = record.get("expires_at") or record.get("vip_expiry")
            if not expires_at and duration_minutes:
                try:
                    base = float(record.get("created_at") or time.time())
                    expires_at = base + (float(duration_minutes) * 60.0)
                except Exception:
                    expires_at = None
            if expires_at:
                try:
                    exp_dt = datetime.fromtimestamp(float(expires_at))
                    lines.append(f"📆 Expires: {exp_dt.strftime('%Y-%m-%d %H:%M:%S')}")
                except Exception:
                    pass
            ...
            purchase_name = _proof_purchase_name(kind, record)
            label = "📚 Method" if kind == "METHOD" else "🛍 Product"
            lines.append(f"{label}: {purchase_name}")
            try:
            ...
            # Show product/service duration or warranty where the record has it.
            duration = str(record.get("duration") or "").strip()
            warranty = str(record.get("warranty") or "").strip()
            if duration and duration.lower() not in ("none", "not specified"):
                lines.append(f"⏳ Duration: {duration}")
            if warranty and warranty.lower() not in ("none", "not specified"):
                lines.append(f"🛡 Warranty: {warranty}")
            try:
                disc = float(record.get("discount_percent") or 0)
            except Exception:
                disc = 0
            if disc > 0:
                lines.append(f"🏷 Discount: {disc:g}%")
...
def reseller_buy_cb(c):
    try:
        ...
        try:
            ...
            _publish_proof('PRODUCT',{'user_id':int(c.from_user.id),'username':c.from_user.username,'amount':price,'product_name':item.get('name'),'quantity':1,'created_at':time.time()})
...
# ---- Public review proof helper (methods + VIP) ----
def _review_user_record(uid):
    try:
        return users_col.find_one({"_id": str(uid)}) or users_col.find_one({"_id": int(uid)}) or users_col.find_one({"user_id": int(uid)}) or {}
    except Exception:
        return {}


def _review_target_name(row):
    rtype = str(row.get("review_type") or "method").lower()
    if rtype == "vip":
        code = str(row.get("plan") or row.get("target_key") or "VIP").replace("vip:", "").upper()
        try:
            plan = (get_subscription_plans() or {}).get(code) or {}
        except Exception:
            plan = {}
        return plan.get("name") or code or "VIP Access"
    fid = row.get("folder_id")
    if fid:
        try:
            from bson import ObjectId
            f = folders_col.find_one({"_id": ObjectId(str(fid))}, {"name": 1})
            if f and f.get("name"):
                return str(f.get("name"))
        except Exception:
            pass
    return str(row.get("target_name") or "Method")


def _publish_review_proof(row):
    target = get_cached_config().get("proof_channel")
    if not target:
        return False
    try:
        ...
        u = _review_user_record(uid)
        username = u.get("username") or row.get("username")
        first_name = u.get("first_name") or row.get("first_name")
        masked = _mask_public_name(username, first_name)
        rating = max(1, min(5, int(row.get("rating") or 0)))
        stars = "⭐" * rating + "☆" * (5 - rating)
        rtype = str(row.get("review_type") or "method").lower()
        target_name = _review_target_name(row)
        lines = [
            "🌟 GLOBEXOMART CUSTOMER REVIEW 🌟",
            "",
            f"👤 Member: @{masked}" if username else f"👤 Member: {masked}",
            f"💎 VIP: {target_name}" if rtype == "vip" else f"📚 Method: {target_name}",
            f"⭐ Rating: {stars} ({rating}/5)",
        ]
        comment = str(row.get("review") or "").strip()
        if comment:
            lines.extend(["", f"💬 Comment: {comment}"])
        lines.extend(["", "💛 Thanks for sharing your experience with GLOBEXOMART! 🚀✨"])
        text = "\n".join(lines)
        try:
            _premium_send_text(target, text)
        except Exception:
        ...
        try: log_event("review_proof_publish_error", row.get("user_id"), details={"error": str(exc)}, level="error")
        except Exception: pass
        return False
...
def rate_method_pick(c):
    ...
    try:
        from bson import ObjectId
        f=folders_col.find_one({"_id":ObjectId(fid)},{"name":1}) or {}
    except Exception:
        f={}
    method_reviews_col.update_one(
        {"user_id":uid,"folder_id":fid,"review_type":{"$in":["method",None]}},
        {"$set":{"rating":n,"review_type":"method","target_name":f.get("name"),"updated_at":time.time(),"status":"draft"},"$setOnInsert":{"created_at":time.time()}},
        upsert=True
    )
    kb=InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("✍️ Add Comment",callback_data=f"reviewtext|{fid}"))
    kb.add(InlineKeyboardButton("✅ Submit Rating Only",callback_data=f"reviewsubmit|method|{fid}"))
    raw_bot.send_message(uid,f"⭐ You selected {n}/5.\n\nAdd an optional comment or submit your star rating now.",reply_markup=kb); bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("reviewtext|"))
def review_text_start(c):
    fid=c.data.split("|",1)[1]; _FEATURE_FLOW[c.from_user.id]={"review_type":"method","review_fid":fid}; msg=raw_bot.send_message(c.from_user.id,"💬 Send a short review (max 500 characters). It will be sent to admin for approval."); bot.register_next_step_handler(msg,review_text_step); bot.answer_callback_query(c.id)

# VIP review flow. The VIP activation message includes this button.
@bot.callback_query_handler(func=lambda c: c.data.startswith("ratevip|"))
def rate_vip_open(c):
    plan=str(c.data.split("|",1)[1] or "VIP").upper(); kb=InlineKeyboardMarkup(row_width=5)
    for n in range(1,6): kb.add(InlineKeyboardButton("⭐"*n,callback_data=f"ratevippick|{plan}|{n}"))
    raw_bot.send_message(c.from_user.id,"💎 RATE YOUR VIP EXPERIENCE\n\nChoose 1–5 stars:",reply_markup=kb); bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("ratevippick|"))
def rate_vip_pick(c):
    _,plan,n=c.data.split("|",2); uid=int(c.from_user.id); n=int(n); plan=plan.upper()
    method_reviews_col.update_one(
        {"user_id":uid,"review_type":"vip","target_key":f"vip:{plan}"},
        {"$set":{"rating":n,"plan":plan,"review_type":"vip","target_key":f"vip:{plan}","updated_at":time.time(),"status":"draft"},"$setOnInsert":{"created_at":time.time()}},
        upsert=True
    )
    kb=InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("✍️ Add Comment",callback_data=f"reviewtextvip|{plan}"))
    kb.add(InlineKeyboardButton("✅ Submit Rating Only",callback_data=f"reviewsubmit|vip|{plan}"))
    raw_bot.send_message(uid,f"⭐ You selected {n}/5 for your VIP experience.\n\nAdd an optional comment or submit your star rating now.",reply_markup=kb); bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("reviewtextvip|"))
def review_vip_text_start(c):
    plan=str(c.data.split("|",1)[1] or "VIP").upper(); _FEATURE_FLOW[c.from_user.id]={"review_type":"vip","review_plan":plan}; msg=raw_bot.send_message(c.from_user.id,"💬 Send a short VIP review (max 500 characters). It will be sent to admin for approval."); bot.register_next_step_handler(msg,review_text_step); bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("reviewsubmit|"))
def review_submit_rating_only(c):
    _,rtype,key=c.data.split("|",2); uid=int(c.from_user.id)
    if rtype=="vip":
        q={"user_id":uid,"review_type":"vip","target_key":f"vip:{key.upper()}"}
    else:
        q={"user_id":uid,"folder_id":key}
    row=method_reviews_col.find_one(q)
    if not row or not row.get("rating"):
        return bot.answer_callback_query(c.id,"Choose a rating first",True)
    method_reviews_col.update_one({"_id":row["_id"]},{"$set":{"status":"pending","review":"","updated_at":time.time()}})
    bot.answer_callback_query(c.id,"Submitted for approval",True)
    raw_bot.send_message(uid,"✅ Your rating was submitted for approval. Thank you! 🌟")


def review_text_step(m):
    st=_FEATURE_FLOW.pop(m.from_user.id,{}) ; rtype=st.get("review_type") or "method"; text=(m.text or "").strip()[:500]
    if not text: return raw_bot.send_message(m.chat.id,"❌ Review cancelled.")
    uid=int(m.from_user.id)
    if rtype=="vip":
        plan=str(st.get("review_plan") or "VIP").upper(); q={"user_id":uid,"review_type":"vip","target_key":f"vip:{plan}"}
    else:
        fid=st.get("review_fid")
        if not fid: return raw_bot.send_message(m.chat.id,"❌ Review cancelled.")
        q={"user_id":uid,"folder_id":fid}
    row=method_reviews_col.find_one(q)
    if not row or not row.get("rating"):
        return raw_bot.send_message(m.chat.id,"❌ Please choose your star rating first.")
    method_reviews_col.update_one({"_id":row["_id"]},{"$set":{"review":text,"status":"pending","updated_at":time.time()}})
    raw_bot.send_message(m.chat.id,"✅ Review submitted for approval. Once approved, it can appear in the GLOBEXOMART proof/review channel. 🌟")

@bot.message_handler(func=lambda m: m.text == "⭐ Reviews" and is_admin(m.from_user.id))
def admin_reviews(m):
    rows=list(method_reviews_col.find({"status":"pending","rating":{"$exists":True}}).sort("updated_at",-1).limit(50)); kb=InlineKeyboardMarkup(row_width=1)
    for r in rows:
        rtype=str(r.get("review_type") or "method").lower(); icon="💎" if rtype=="vip" else "📚"; name=_review_target_name(r); comment=str(r.get("review") or "")
        kb.add(InlineKeyboardButton(f"{icon} ⭐ {r.get('rating',0)}/5 • {name[:25]} • {comment[:22] or 'No comment'}",callback_data=f"reviewadm|{r['_id']}"))
    raw_bot.send_message(m.chat.id,"⭐ REVIEWS PENDING APPROVAL\n\nMethod and VIP reviews appear here before being shared to the proof channel.",reply_markup=kb if rows else None)
...
def review_admin_open(c):
    ...
    if not r:return bot.answer_callback_query(c.id,"Not found",True)
    u=_review_user_record(r.get("user_id")); uname=u.get("username") or "No username"; name=_review_target_name(r); rtype=str(r.get("review_type") or "method").lower(); kind="VIP" if rtype=="vip" else "Method"
    kb=InlineKeyboardMarkup(row_width=2); kb.add(InlineKeyboardButton("✅ Approve + Publish",callback_data=f"reviewset|{r['_id']}|approved"),InlineKeyboardButton("❌ Reject",callback_data=f"reviewset|{r['_id']}|rejected"))
    raw_bot.send_message(c.from_user.id,f"⭐ REVIEW\n\nType: {kind}\nItem: {name}\nRating: {r.get('rating')}/5\nUser: @{uname}\nUser ID: {r.get('user_id')}\n\n💬 {r.get('review') or 'No comment provided'}",reply_markup=kb);bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("reviewset|") and is_admin(c.from_user.id))
def review_admin_set(c):
    from bson import ObjectId
    _,oid,status=c.data.split("|",2); row=method_reviews_col.find_one({"_id":ObjectId(oid)})
    if not row:return bot.answer_callback_query(c.id,"Review not found",True)
    method_reviews_col.update_one({"_id":row["_id"]},{"$set":{"status":status,"reviewed_by":c.from_user.id,"reviewed_at":time.time()}})
    if status=="approved":
        row["status"]="approved"; _publish_review_proof(row)
        bot.answer_callback_query(c.id,"Approved and published",True)
        try: raw_bot.send_message(int(row.get("user_id")),"✅ Your review was approved and shared. Thanks for your feedback! 🌟💛")
        except Exception: pass
    else:
        bot.answer_callback_query(c.id,"Rejected",True)
...
def v26_payment_profiles(m):
    rows=list(payment_profiles_col.find({}).sort([('active',-1),('name',1)]))
    kb=InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("➕ Add Payment Method",callback_data='v26payprof|add'))
    for r in rows[:40]:
        state='✅' if r.get('active',True) else '⛔'
        kind=str(r.get('method_type') or r.get('coin') or 'Payment')
        kb.add(InlineKeyboardButton(f"{state} {r.get('name')} — {kind}",callback_data=f"v26payprof|view|{r['_id']}"))
    raw_bot.send_message(m.chat.id,"💳 MULTIPLE PAYMENT METHODS\n\nAdd crypto, exchange, bank, mobile-wallet or other manual payment options. Every active method is shown during checkout.\n\nExamples: USDT TRC20, USDT BEP20, Binance Pay, bank transfer, EasyPaisa/JazzCash, etc.",reply_markup=kb)
...
def v26payprof_cb(c):
    ...
    action=p[1]
    if action=='add':
        msg=raw_bot.send_message(c.from_user.id,"Send payment method in this format:\n\nNAME | TYPE | NETWORK/PROVIDER | ADDRESS/ACCOUNT/DETAILS | REGION\n\nExample:\nBinance Pay | Exchange | Binance Pay | 123456789 | ALL\n\nUse ALL for worldwide.")
        bot.register_next_step_handler(msg,v26payprof_add);return bot.answer_callback_query(c.id)
    if len(p)<3:return bot.answer_callback_query(c.id,"Invalid action",True)
    try:r=payment_profiles_col.find_one({'_id':ObjectId(p[2])})
    except Exception:r=None
    if not r:return bot.answer_callback_query(c.id,"Not found",True)
    if action=='view':
        kb=InlineKeyboardMarkup(row_width=2)
        kb.row(InlineKeyboardButton('⛔ Disable' if r.get('active',True) else '✅ Enable',callback_data=f"v26payprof|toggle|{r['_id']}"),InlineKeyboardButton('🗑 Delete',callback_data=f"v26payprof|delete|{r['_id']}"))
        kb.add(InlineKeyboardButton('✏️ Replace Details',callback_data=f"v26payprof|edit|{r['_id']}"))
        txt=f"💳 {r.get('name')}\n\nType: {r.get('method_type') or r.get('coin','Payment')}\nProvider/Network: {r.get('network') or '-'}\nDetails: {r.get('address') or '-'}\nRegion: {r.get('country') or 'ALL'}\nStatus: {'ACTIVE' if r.get('active',True) else 'DISABLED'}"
        raw_bot.send_message(c.from_user.id,txt,reply_markup=kb);return bot.answer_callback_query(c.id)
    if action=='toggle':
        payment_profiles_col.update_one({'_id':r['_id']},{'$set':{'active':not r.get('active',True),'updated_at':time.time()}})
        return bot.answer_callback_query(c.id,'Updated',True)
    if action=='delete':
        payment_profiles_col.delete_one({'_id':r['_id']});bot.answer_callback_query(c.id,'Deleted',True)
        try:bot.delete_message(c.message.chat.id,c.message.message_id)
        except Exception:pass
        return
    if action=='edit':
        msg=raw_bot.send_message(c.from_user.id,"Send replacement as:\nNAME | TYPE | NETWORK/PROVIDER | ADDRESS/ACCOUNT/DETAILS | REGION")
        bot.register_next_step_handler(msg,v28_payment_profile_edit,str(r['_id']));return bot.answer_callback_query(c.id)

def _v28_parse_payment_profile(text):
    q=[x.strip() for x in (text or '').split('|')]
    if len(q)<5:raise ValueError('Use 5 fields separated by |')
    if not q[0] or not q[3]:raise ValueError('Name and payment details are required')
    return {'name':q[0][:80],'method_type':q[1][:40] or 'Payment','coin':q[1][:40] or 'Payment','network':q[2][:80],'address':q[3][:500],'country':(q[4][:50] or 'ALL').upper(),'active':True,'updated_at':time.time()}

def v26payprof_add(m):
    try:
        doc=_v28_parse_payment_profile(m.text);doc['created_at']=time.time();payment_profiles_col.insert_one(doc)
        raw_bot.send_message(m.chat.id,"✅ Payment method saved and available for checkout.",reply_markup=admin_menu())
    except Exception as exc:raw_bot.send_message(m.chat.id,f"❌ {exc}",reply_markup=admin_menu())

def v28_payment_profile_edit(m,pid):
    try:
        doc=_v28_parse_payment_profile(m.text);payment_profiles_col.update_one({'_id':ObjectId(pid)},{'$set':doc})
        raw_bot.send_message(m.chat.id,"✅ Payment method updated.",reply_markup=admin_menu())
    except Exception as exc:raw_bot.send_message(m.chat.id,f"❌ {exc}",reply_markup=admin_menu())
...
def _usdt_instructions(amount):
    ...
    amount_text = f"${float(amount):g} USDT" if float(amount or 0) > 0 else "Enter the amount requested by this checkout"
    lines=[f"💳 PAYMENT OPTIONS\nAmount: {amount_text}"]
    for i,r in enumerate(rows[:12],1):
        country=str(r.get('country') or 'ALL').upper()
        kind=str(r.get('method_type') or r.get('coin') or 'Payment')
        provider=str(r.get('network') or '').strip()
        title=f"{i}. {r.get('name')} — {kind}" + (f" / {provider}" if provider else "")
        lines.append(f"\n{title}\n{r.get('address')}\nRegion: {country}")
    lines.append("\nChoose any active method above, send the exact required amount, then submit the payment reference/TxID and proof where requested.")
    return '\n'.join(lines)
...
# =========================
# 🏢 V28 PLATFORM / GROWTH / OPERATIONS SUITE
# =========================
# Consolidates the next feature wave into hubs so the main menu stays usable.
# Existing V1-V27 handlers remain untouched.

method_progress_col = db["method_progress"]
method_notes_col = db["method_notes"]
user_inbox_col = db["user_inbox"]
automation_jobs_col = db["automation_jobs_v28"]
security_events_col = db["security_events_v28"]
affiliate_codes_col = db["affiliate_codes_v28"]
finance_statements_col = db["finance_statements_v28"]
product_interest_col = db["product_interest_v28"]

for _c,_idx in [
    (method_progress_col,[("user_id",1),("folder_number",1)]),
    (method_notes_col,[("user_id",1),("folder_number",1)]),
    (user_inbox_col,[("user_id",1),("created_at",-1)]),
    (automation_jobs_col,[("active",1),("run_at",1)]),
    (security_events_col,[("created_at",-1),("user_id",1)]),
    (affiliate_codes_col,[("user_id",1),("code",1)]),
    (product_interest_col,[("product_id",1),("user_id",1)]),
]:
    try:_c.create_index(_idx)
    except Exception:pass


def _v28_user_inbox(uid,title,text,kind="info"):
    try:user_inbox_col.insert_one({'user_id':int(uid),'title':str(title)[:100],'text':str(text)[:3000],'kind':kind,'read':False,'created_at':time.time()})
    except Exception:pass


def _v28_method_candidates(uid,limit=15):
    rows=list(folders_col.find({'parent':None}).sort('created_at',-1).limit(limit))
    return rows


@bot.message_handler(func=lambda m:m.text=='🧰 More')
def v28_more_hub(m):
    raw_bot.send_message(
        m.chat.id,
        '🧰 MORE\n\nAll extra tools are organized here to keep your main menu clean. Choose any option below.',
        reply_markup=more_menu(m.from_user.id),
    )

@bot.message_handler(func=lambda m:m.text=='⬅️ Main Menu')
def v29_more_back(m):
    raw_bot.send_message(m.chat.id,'✅ Main menu',reply_markup=main_menu(m.from_user.id))

@bot.message_handler(func=lambda m:m.text=='✨ More Tools')
def v29_extra_more_tools(m):
    kb=InlineKeyboardMarkup(row_width=2)
    kb.row(InlineKeyboardButton('💎 VIP Compare',callback_data='v28more|vipcompare'),InlineKeyboardButton('📈 My Progress',callback_data='v28more|progress'))
    kb.row(InlineKeyboardButton('📝 Method Notes',callback_data='v28more|notes'),InlineKeyboardButton('📥 Notification Inbox',callback_data='v28more|inbox'))
    kb.row(InlineKeyboardButton('🤝 Affiliate Dashboard',callback_data='v28more|affiliate'),InlineKeyboardButton('🛍 Trending',callback_data='v28more|trending'))
    kb.row(InlineKeyboardButton('🔔 Preferences',callback_data='v28more|prefs'),InlineKeyboardButton('🧭 My Hub',callback_data='v28more|myhub'))
    raw_bot.send_message(m.chat.id,'✨ MORE TOOLS\n\nLearning progress, VIP comparison, private notes, alerts and affiliate tools.',reply_markup=kb)


@bot.callback_query_handler(func=lambda c:str(c.data or '').startswith('v28more|'))
def v28_more_cb(c):
    a=c.data.split('|')[1];uid=c.from_user.id
    if a=='vipcompare':
        lines=['💎 VIP PLAN COMPARISON','']
        plans=get_subscription_plans()
        for code,row in plans.items():
            price=float(row.get('price',0) or 0); mins=int(row.get('duration_minutes',int(row.get('days',30))*1440) or 0)
            days=max(1,mins//1440);monthly=price/max(days/30,1)
            lines.append(f"• {row.get('name',code)} — {days} days — ${price:g} (≈ ${monthly:.2f}/30 days)")
        lines.append('\nVIP includes access to Free + VIP methods during the active subscription and the benefits configured by admin.')
        raw_bot.send_message(uid,'\n'.join(lines));return bot.answer_callback_query(c.id)
    if a=='progress':
        rows=_v28_method_candidates(uid);kb=InlineKeyboardMarkup(row_width=1)
        for r in rows:kb.add(InlineKeyboardButton(f"{r.get('name','Method')[:45]}",callback_data=f"v28prog|open|{r.get('number')}"))
        raw_bot.send_message(uid,'📈 MY METHOD PROGRESS\n\nChoose a method to mark Not Started / Learning / Completed.',reply_markup=kb);return bot.answer_callback_query(c.id)
    if a=='notes':
        msg=raw_bot.send_message(uid,'📝 METHOD NOTES\n\nSend METHOD NUMBER | YOUR PRIVATE NOTE\nExample: 12 | Need to test step 3 tomorrow')
        bot.register_next_step_handler(msg,v28_note_save);return bot.answer_callback_query(c.id)
    if a=='inbox':
        rows=list(user_inbox_col.find({'user_id':int(uid)}).sort('created_at',-1).limit(12));lines=['📥 NOTIFICATION INBOX','']
        if not rows:lines.append('No saved notifications yet.')
        for r in rows:
            mark='🆕' if not r.get('read') else '•';lines.append(f"{mark} {r.get('title')}\n{r.get('text','')[:180]}")
        user_inbox_col.update_many({'user_id':int(uid),'read':False},{'$set':{'read':True}});raw_bot.send_message(uid,'\n\n'.join(lines));return bot.answer_callback_query(c.id)
    if a=='affiliate':
        u=users_col.find_one({'_id':str(uid)}) or {};refs=int(u.get('refs',0) or 0);bal=float(u.get('referral_balance_usdt',0) or 0);earned=float(u.get('referral_total_earned_usdt',0) or 0)
        sales=referral_sales_col.count_documents({'referrer_id':{'$in':[uid,str(uid)]}}) if 'referral_sales_col' in globals() else 0
        raw_bot.send_message(uid,f"🤝 AFFILIATE DASHBOARD\n\nVerified referrals: {refs}\nAttributed purchases: {sales}\nAvailable commission: ${bal:.2f}\nLifetime commission: ${earned:.2f}\n\nUse your Referral section for your tracked invite link.");return bot.answer_callback_query(c.id)
    if a=='trending':
        rows=list(shop_products_col.find({'active':{'$ne':False}}).sort([('sales_count',-1),('created_at',-1)]).limit(10));lines=['🔥 TRENDING PRODUCTS','']
        for r in rows:lines.append(f"• {r.get('name','Product')} — ${float(r.get('price',0) or 0):g}")
        raw_bot.send_message(uid,'\n'.join(lines) if len(lines)>2 else '🔥 No trending products yet.');return bot.answer_callback_query(c.id)
    if a=='prefs':
        try:v26_prefs(c.message)
        except Exception:raw_bot.send_message(uid,'Use 🔔 Preferences from the menu.')
        return bot.answer_callback_query(c.id)
    if a=='myhub':
        try:v27_my_hub(c.message)
        except Exception:raw_bot.send_message(uid,'Use 🧭 My Hub from the menu.')
        return bot.answer_callback_query(c.id)
    bot.answer_callback_query(c.id)


@bot.callback_query_handler(func=lambda c:str(c.data or '').startswith('v28prog|'))
def v28_progress_cb(c):
    p=c.data.split('|');uid=c.from_user.id
    if p[1]=='open':
        num=int(p[2]);r=fs.get_by_number(num)
        if not r:return bot.answer_callback_query(c.id,'Method not found',True)
        cur=method_progress_col.find_one({'user_id':int(uid),'folder_number':num}) or {};status=cur.get('status','Not Started')
        kb=InlineKeyboardMarkup(row_width=3)
        for st,label in [('not','Not Started'),('learn','Learning'),('done','Completed')]:kb.add(InlineKeyboardButton(('✅ ' if status==label else '')+label,callback_data=f"v28prog|set|{num}|{st}"))
        raw_bot.send_message(uid,f"📈 {r.get('name')}\nCurrent status: {status}",reply_markup=kb);return bot.answer_callback_query(c.id)
    if p[1]=='set':
        num=int(p[2]);mapping={'not':'Not Started','learn':'Learning','done':'Completed'};st=mapping.get(p[3],'Not Started')
        method_progress_col.update_one({'user_id':int(uid),'folder_number':num},{'$set':{'status':st,'updated_at':time.time()}},upsert=True)
        bot.answer_callback_query(c.id,f'Set to {st}',True)


def v28_note_save(m):
    try:
        num_s,note=(m.text or '').split('|',1);num=int(num_s.strip());r=fs.get_by_number(num)
        if not r:raise ValueError('Method number not found')
        method_notes_col.update_one({'user_id':int(m.from_user.id),'folder_number':num},{'$set':{'note':note.strip()[:3500],'updated_at':time.time(),'method_name':r.get('name')}},upsert=True)
        raw_bot.send_message(m.chat.id,'✅ Private method note saved.',reply_markup=main_menu(m.from_user.id))
    except Exception as exc:raw_bot.send_message(m.chat.id,f'❌ {exc}\nUse: METHOD NUMBER | NOTE')


# -------- Admin Platform Center --------
_v28_admin_menu_original=admin_menu
def admin_menu():
    kb=_v28_admin_menu_original()
    try:kb.row('🏢 Platform Center')
    except Exception:pass
    return kb


@bot.message_handler(func=lambda m:m.text=='🏢 Platform Center' and is_admin(m.from_user.id))
def v28_platform_center(m):
    kb=ReplyKeyboardMarkup(resize_keyboard=True,row_width=2)
    rows=[('📊 Analytics Center','💵 Finance Center'),('📦 Inventory Center','⚙️ Automation Center'),('🛡 Security Center','🔎 Universal Search'),('🤝 Affiliate Center','🛒 Sales Center'),('💳 Payment Profiles','↕️ Arrange Main Buttons'),('🚀 Advanced Controls','⚙️ ADMIN PANEL')]
    for row in rows:kb.row(*row)
    raw_bot.send_message(m.chat.id,'🏢 GLOBEXOMART PLATFORM CENTER\n\nCentralized sales, analytics, automation, finance, inventory and security controls.',reply_markup=kb)


@bot.message_handler(func=lambda m:m.text=='📊 Analytics Center' and is_admin(m.from_user.id))
def v28_analytics_center(m):
    users=users_col.count_documents({});vip=users_col.count_documents({'vip':True});products=shop_products_col.count_documents({'active':{'$ne':False}});methods=folders_col.count_documents({})
    orders=shop_orders_col.count_documents({});paid=shop_orders_col.count_documents({'status':{'$in':['paid','approved','delivered','completed']}})
    payments=payments_col.count_documents({});approved=payments_col.count_documents({'status':{'$in':['approved','paid']}})
    searches=search_analytics_col.count_documents({}) if 'search_analytics_col' in globals() else 0
    checkout=checkout_intents_col.count_documents({}) if 'checkout_intents_col' in globals() else 0
    raw_bot.send_message(m.chat.id,f"📊 ANALYTICS CENTER\n\nUsers: {users:,}\nActive VIP: {vip:,}\nMethods: {methods:,}\nProducts: {products:,}\nOrders: {orders:,} ({paid:,} completed)\nPayments: {payments:,} ({approved:,} approved)\nTracked checkouts: {checkout:,}\nSearches: {searches:,}\n\nFor detailed conversion, retention, method and campaign analytics use the existing Funnel, Retention, Method Analytics and Business Dashboard tools.")


@bot.message_handler(func=lambda m:m.text=='💵 Finance Center' and is_admin(m.from_user.id))
def v28_finance_center(m):
    ledger=list(financial_ledger_col.find({}).sort('created_at',-1).limit(200)) if 'financial_ledger_col' in globals() else []
    income=sum(float(x.get('amount',0) or 0) for x in ledger if float(x.get('amount',0) or 0)>0);out=sum(abs(float(x.get('amount',0) or 0)) for x in ledger if float(x.get('amount',0) or 0)<0)
    refunds=float(sum(float(x.get('amount',0) or 0) for x in refunds_col.find({'status':{'$ne':'cancelled'}}))) if 'refunds_col' in globals() else 0
    raw_bot.send_message(m.chat.id,f"💵 FINANCE CENTER\n\nRecent ledger income: ${income:.2f}\nRecent ledger outflow: ${out:.2f}\nRecorded refunds: ${refunds:.2f}\n\nOpen Financial Ledger, Profit Tracking, Revenue Manager, Refund Manager or Export Center for detailed accounting.")


@bot.message_handler(func=lambda m:m.text=='📦 Inventory Center' and is_admin(m.from_user.id))
def v28_inventory_center(m):
    rows=list(shop_products_col.find({'active':{'$ne':False}}).sort('created_at',-1).limit(30));lines=['📦 INVENTORY CENTER','']
    low=0
    for r in rows:
        stock=r.get('stock')
        if stock is None:stock=r.get('quantity')
        stock_txt='Unlimited/Not tracked' if stock is None else str(stock)
        try:
            if stock is not None and int(stock)<=5:low+=1
        except Exception:pass
        lines.append(f"• {r.get('name','Product')} — Stock: {stock_txt}")
    lines.append(f"\nLow-stock items (≤5): {low}")
    raw_bot.send_message(m.chat.id,'\n'.join(lines)[:3900])


@bot.message_handler(func=lambda m:m.text=='🛡 Security Center' and is_admin(m.from_user.id))
def v28_security_center(m):
    blocked=users_col.count_documents({'submission_blocked':True})+users_col.count_documents({'submissions_blocked':True})
    dup_tx=list(payments_col.aggregate([{'$match':{'transaction_id':{'$nin':[None,'']}}},{'$group':{'_id':'$transaction_id','n':{'$sum':1}}},{'$match':{'n':{'$gt':1}}},{'$count':'n'}]))
    dups=int(dup_tx[0]['n']) if dup_tx else 0
    risks=risk_events_col.count_documents({}) if 'risk_events_col' in globals() else 0
    raw_bot.send_message(m.chat.id,f"🛡 SECURITY CENTER\n\nSubmission-blocked users: {blocked}\nDuplicate TxID groups: {dups}\nRisk events: {risks}\n\nUse Risk Panel, Rate Limits, Staff Audit, Admin Safety, Backup/Restore and Emergency Mode for enforcement and recovery.")


@bot.message_handler(func=lambda m:m.text=='🔎 Universal Search' and is_admin(m.from_user.id))
def v28_universal_search_start(m):
    msg=raw_bot.send_message(m.chat.id,'🔎 UNIVERSAL SEARCH\n\nSend username, user ID, TxID, invoice/order ID, product name, or method name.')
    bot.register_next_step_handler(msg,v28_universal_search_run)


def v28_universal_search_run(m):
    q=(m.text or '').strip();ql=q.lower();lines=[f"🔎 RESULTS FOR: {q}",'']
    # users
    uq={'$or':[{'_id':q},{'username':{'$regex':re.escape(q.lstrip('@')),'$options':'i'}}]}
    for u in users_col.find(uq).limit(5):lines.append(f"👤 User: @{u.get('username') or '-'} | ID {u.get('_id')} | VIP {'Yes' if u.get('vip') else 'No'}")
    # payments
    for r in payments_col.find({'$or':[{'transaction_id':{'$regex':re.escape(q),'$options':'i'}},{'invoice_id':{'$regex':re.escape(q),'$options':'i'}}]}).limit(5):lines.append(f"💳 Payment: {r.get('type')} | {r.get('status')} | ${float(r.get('amount',0) or 0):g} | {r.get('transaction_id','-')}")
    # products/methods
    for r in shop_products_col.find({'name':{'$regex':re.escape(q),'$options':'i'}}).limit(5):lines.append(f"🛍 Product: {r.get('name')} | ${float(r.get('price',0) or 0):g}")
    for r in folders_col.find({'name':{'$regex':re.escape(q),'$options':'i'}}).limit(5):lines.append(f"📚 Method: {r.get('name')} | internal #{r.get('number')}")
    raw_bot.send_message(m.chat.id,'\n'.join(lines)[:3900] if len(lines)>2 else 'No matching records.',reply_markup=admin_menu())


@bot.message_handler(func=lambda m:m.text=='⚙️ Automation Center' and is_admin(m.from_user.id))
def v28_automation_center(m):
    auto_posts=auto_posts_col.count_documents({'active':True}) if 'auto_posts_col' in globals() else 0
    auto_b=auto_broadcasts_col.count_documents({'active':True}) if 'auto_broadcasts_col' in globals() else 0
    smart=smart_broadcasts_col.count_documents({'status':{'$in':['scheduled','pending']}}) if 'smart_broadcasts_col' in globals() else 0
    drops=scheduled_drops_col.count_documents({'status':{'$in':['scheduled','pending']}}) if 'scheduled_drops_col' in globals() else 0
    raw_bot.send_message(m.chat.id,f"⚙️ AUTOMATION CENTER\n\nActive Auto Posts: {auto_posts}\nActive Auto Broadcasts: {auto_b}\nScheduled Smart Broadcasts: {smart}\nScheduled Content Drops: {drops}\n\nUse Auto Posts, Auto Broadcast, Smart Broadcast, Scheduled Drops, VIP expiry reminders and Reports to manage automations.")


@bot.message_handler(func=lambda m:m.text=='🤝 Affiliate Center' and is_admin(m.from_user.id))
def v28_affiliate_center(m):
    refs=users_col.count_documents({'refs':{'$gt':0}});sales=referral_sales_col.count_documents({}) if 'referral_sales_col' in globals() else 0;pending=referral_withdrawals_col.count_documents({'status':'pending'}) if 'referral_withdrawals_col' in globals() else 0
    raw_bot.send_message(m.chat.id,f"🤝 AFFILIATE CENTER\n\nUsers with referrals: {refs}\nAttributed referral sales: {sales}\nPending affiliate withdrawals: {pending}\n\nUse Affiliate Manager, Campaign Manager, Referral Settings, Leaderboard and Ref Withdrawals for management.")


@bot.message_handler(func=lambda m:m.text=='🛒 Sales Center' and is_admin(m.from_user.id))
def v28_sales_center(m):
    deals=deals_col.count_documents({'active':True}) if 'deals_col' in globals() else 0;coupons=coupons_col.count_documents({'active':True}) if 'coupons_col' in globals() else 0;bundles=bundles_col.count_documents({'active':True}) if 'bundles_col' in globals() else 0
    raw_bot.send_message(m.chat.id,f"🛒 SALES CENTER\n\nActive deals: {deals}\nActive coupons: {coupons}\nActive bundles: {bundles}\n\nSales tools already available: Deals, Coupons, Bundles, Personalized For You, Campaign Tracking, Affiliate/KOL, abandoned checkout recovery, promotions, loyalty/cashback, Gift VIP/Product and renewal flows.")


# Build/version marker.
BOT_BUILD_VERSION='28.0'
...
# =========================
# 📦 PRODUCT PREORDER SYSTEM
# =========================
_preorder_admin_state = {}

def _preorder_money(v):
    try: return f"${float(v):.2f} USDT"
    except Exception: return "$0.00 USDT"

@bot.callback_query_handler(func=lambda c: c.data.startswith("shopadm_preorder|"))
def shop_admin_toggle_preorder(c):
    if not is_admin(c.from_user.id):
        return bot.answer_callback_query(c.id, "Admin only", True)
    p=_shop_product(c.data.split("|",1)[1])
    if not p or p.get("kind")!="paid":
        return bot.answer_callback_query(c.id, "Paid product not found", True)
    val=not bool(p.get("preorder_enabled",False))
    shop_products_col.update_one({"_id":p["_id"]},{"$set":{"preorder_enabled":val,"updated_at":time.time()}})
    bot.answer_callback_query(c.id, "Preorder enabled" if val else "Preorder disabled", True)

@bot.callback_query_handler(func=lambda c: c.data.startswith("preorderask|"))
def preorder_ask(c):
    if force_block(c.from_user.id): return
    p=_shop_product(c.data.split("|",1)[1])
    if not p or p.get("kind")!="paid" or not p.get("preorder_enabled") or not _shop_is_available(p):
        return bot.answer_callback_query(c.id,"Preorder is not available",True)
    _product_qty_state[c.from_user.id]={"product_id":str(p["_id"]),"preorder":True}
    msg=raw_bot.send_message(c.from_user.id,
        f"📦 PREORDER — {p.get('name')}\n\n"
        f"Price each: {_shop_money(p.get('price_usdt',0))}\n"
        f"Expected delivery: {p.get('preorder_eta') or 'To be announced'}\n\n"
        "This is an advance payment preorder. Your approved USDT bot balance is charged now, and the order stays in Pending Preorders until admin fulfills it.\n\n"
        "Send quantity to preorder (1-100), or /cancel.")
    bot.register_next_step_handler(msg, preorder_quantity_received)
    bot.answer_callback_query(c.id,"Send quantity")

def preorder_quantity_received(m):
    uid=m.from_user.id; st=_product_qty_state.get(uid)
    if not st or not st.get("preorder"): return
    text=(m.text or "").strip()
    if text.lower()=="/cancel":
        _product_qty_state.pop(uid,None); return raw_bot.send_message(uid,"❌ Preorder cancelled.",reply_markup=main_menu(uid))
    try:
        qty=int(text)
        if qty<1 or qty>100: raise ValueError()
    except Exception:
        msg=raw_bot.send_message(uid,"❌ Send a quantity from 1 to 100, or /cancel.")
        return bot.register_next_step_handler(msg,preorder_quantity_received)
    p=_shop_product(st["product_id"])
    if not p or not p.get("preorder_enabled") or not _shop_is_available(p):
        _product_qty_state.pop(uid,None); return raw_bot.send_message(uid,"❌ This preorder is no longer available.")
    each=float(p.get("price_usdt",0) or 0); subtotal=round(each*qty,2)
    disc=max(_shop_discount(qty), _active_product_deal_discount(str(p.get("_id"))), _active_coupon_discount(uid,"product",str(p.get("_id"))))
    total=round(subtotal*(100-disc)/100.0,2)
    _product_qty_state.pop(uid,None)
    kb=InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("✅ Pay & Place Preorder",callback_data=f"preorderconfirm|{p['_id']}|{qty}"))
    kb.add(InlineKeyboardButton("❌ Cancel",callback_data="shopcancel"))
    raw_bot.send_message(uid,
        f"📦 CONFIRM PREORDER\n\nProduct: {p.get('name')}\nQuantity: {qty}\n"
        f"Price each: {_shop_money(each)}\nSubtotal: {_shop_money(subtotal)}\nDiscount: {disc}%\n"
        f"Pay now: {_shop_money(total)}\nYour balance: {_shop_money(_user_usdt_balance(uid))}\n"
        f"Expected: {p.get('preorder_eta') or 'To be announced'}\n\n"
        "Payment is taken in advance. Delivery happens later when admin fulfills the preorder.",reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data.startswith("preorderconfirm|"))
def preorder_confirm(c):
    if force_block(c.from_user.id): return
    try:
        _,pid,qtys=c.data.split("|",2); qty=int(qtys)
        if qty<1 or qty>100: raise ValueError()
    except Exception:
        return bot.answer_callback_query(c.id,"Invalid preorder",True)
    uid=int(c.from_user.id); p=_shop_product(pid)
    if not p or p.get("kind")!="paid" or not p.get("preorder_enabled") or not _shop_is_available(p):
        return bot.answer_callback_query(c.id,"Preorder is closed",True)
    each=float(p.get("price_usdt",0) or 0); subtotal=round(each*qty,2)
    disc=max(_shop_discount(qty), _active_product_deal_discount(str(p.get("_id"))), _active_coupon_discount(uid,"product",str(p.get("_id"))))
    total=round(subtotal*(100-disc)/100.0,2)
    if total<=0: return bot.answer_callback_query(c.id,"Price is not configured",True)
    debit=users_col.update_one({"_id":str(uid),"usdt_balance":{"$gte":total}},{"$inc":{"usdt_balance":-total}})
    if debit.modified_count!=1:
        raw_bot.send_message(uid,f"❌ Insufficient USDT balance.\nNeed: {_shop_money(total)}\nBalance: {_shop_money(_user_usdt_balance(uid))}\n\nUse 💳 Deposit first.")
        return bot.answer_callback_query(c.id,"Insufficient balance",True)
    try:
        doc={
            "user_id":uid,"chat_id":int(c.message.chat.id),"username":c.from_user.username,
            "product_id":str(p["_id"]),"product_name":p.get("name"),"quantity":qty,
            "price_each":each,"subtotal":subtotal,"discount_percent":disc,"total":total,
            "paid_with":"USDT Balance","payment_status":"approved","status":"pending_fulfillment",
            "expected_delivery":p.get("preorder_eta") or "To be announced",
            "created_at":time.time(),"approved_at":time.time()
        }
        rid=preorders_col.insert_one(doc).inserted_id
        shop_products_col.update_one({"_id":p["_id"]},{"$inc":{"sales":qty,"revenue":float(total)},"$set":{"updated_at":time.time()}})
        payments_col.insert_one({"user_id":uid,"type":"product_preorder","preorder_id":str(rid),"product_id":str(p["_id"]),"amount":float(total),"currency":"USDT","mode":"balance","status":"paid","created_at":time.time()})
        _consume_coupon(uid)
        try: _loyalty_after_purchase(uid,float(total),"product_preorder",str(rid))
        except Exception: pass
        short=str(rid)[-8:].upper()
        _send_globexomart_invoice(uid,"PRODUCT PREORDER",short,[
            ("Preorder ID",f"#{short}"),("Product",p.get("name") or "Product"),("Quantity",qty),
            ("Purchase Date",datetime.now().strftime("%Y-%m-%d %H:%M:%S")),("Expected Delivery",doc["expected_delivery"]),
            ("Price Each",_shop_money(each)),("Subtotal",_shop_money(subtotal)),("Discount",f"{disc}%" if disc else "No discount"),
            ("Paid in Advance",_shop_money(total)),("Paid With","USDT Balance"),("Status","Paid — Awaiting Fulfillment"),("Bot","@globexomartbot")
        ],status="PREORDER PAID / AWAITING FULFILLMENT")
        raw_bot.send_message(uid,f"✅ PREORDER CONFIRMED\n\nPreorder: #{short}\nProduct: {p.get('name')}\nQuantity: {qty}\nPaid: {_shop_money(total)}\nExpected: {doc['expected_delivery']}\n\nYour payment is approved and your preorder is now waiting for fulfillment. You will receive the product here when admin completes it.")
        for adm in get_all_admins():
            try: raw_bot.send_message(int(adm["_id"]),f"📦 NEW PAID PREORDER\n\n#{short}\nUser: @{c.from_user.username or 'None'} ({uid})\nProduct: {p.get('name')} ×{qty}\nPaid: {_shop_money(total)}\nStatus: Pending fulfillment")
            except Exception: pass
        bot.answer_callback_query(c.id,"Preorder placed",True)
    except Exception as exc:
        users_col.update_one({"_id":str(uid)},{"$inc":{"usdt_balance":total}})
        log_event("preorder_create_error",uid,details={"error":str(exc),"product_id":pid},level="error")
        raw_bot.send_message(uid,"❌ Preorder could not be created. Your balance was restored.")


def _preorder_admin_keyboard(row):
    kb=InlineKeyboardMarkup(row_width=2); rid=str(row["_id"])
    kb.add(InlineKeyboardButton("✅ Fulfill",callback_data=f"preorder_fulfill|{rid}"),InlineKeyboardButton("↩️ Refund",callback_data=f"preorder_refundask|{rid}"))
    kb.add(InlineKeyboardButton("⬅️ Pending Preorders",callback_data="preorder_admin_list"))
    return kb

def _send_pending_preorders(uid):
    rows=list(preorders_col.find({"payment_status":"approved","status":"pending_fulfillment"}).sort("created_at",1).limit(100))
    kb=InlineKeyboardMarkup(row_width=1)
    for r in rows:
        name=(r.get("username") and "@"+r.get("username")) or str(r.get("user_id"))
        kb.add(InlineKeyboardButton(f"📦 {name} • {r.get('product_name','Product')[:25]} ×{r.get('quantity',1)} • {_preorder_money(r.get('total',0))}",callback_data=f"preorder_view|{r['_id']}"))
    text=f"📦 PENDING PREORDERS\n\nApproved + paid preorders waiting for delivery: {len(rows)}"
    if not rows: text+="\n\n✅ No pending preorders."
    raw_bot.send_message(uid,text,reply_markup=kb)

@bot.message_handler(func=lambda m: m.text=="📦 Pending Preorders" and is_admin(m.from_user.id))
def pending_preorders_button(m):
    _send_pending_preorders(m.from_user.id)

@bot.callback_query_handler(func=lambda c: c.data=="preorder_admin_list")
def pending_preorders_cb(c):
    if not is_admin(c.from_user.id): return bot.answer_callback_query(c.id,"Admin only",True)
    _send_pending_preorders(c.from_user.id); bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("preorder_view|"))
def preorder_view_admin(c):
    if not is_admin(c.from_user.id): return bot.answer_callback_query(c.id,"Admin only",True)
    try: rid=ObjectId(c.data.split("|",1)[1])
    except Exception: return bot.answer_callback_query(c.id,"Invalid preorder",True)
    r=preorders_col.find_one({"_id":rid})
    if not r: return bot.answer_callback_query(c.id,"Not found",True)
    dt=datetime.fromtimestamp(float(r.get("created_at",time.time()))).strftime("%Y-%m-%d %H:%M:%S")
    text=(f"📦 PREORDER DETAILS\n\nID: #{str(r['_id'])[-8:].upper()}\nUser: @{r.get('username') or 'None'}\nUser ID: {r.get('user_id')}\n"
          f"Product: {r.get('product_name')}\nQuantity: {r.get('quantity',1)}\nPaid: {_shop_money(r.get('total',0))}\n"
          f"Payment: {r.get('payment_status')}\nStatus: {r.get('status')}\nExpected: {r.get('expected_delivery')}\nCreated: {dt}")
    raw_bot.send_message(c.from_user.id,text,reply_markup=_preorder_admin_keyboard(r)); bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("preorder_fulfill|"))
def preorder_fulfill_start(c):
    if not is_admin(c.from_user.id): return bot.answer_callback_query(c.id,"Admin only",True)
    try: rid=ObjectId(c.data.split("|",1)[1])
    except Exception: return bot.answer_callback_query(c.id,"Invalid",True)
    r=preorders_col.find_one({"_id":rid,"status":"pending_fulfillment"})
    if not r: return bot.answer_callback_query(c.id,"Already processed or missing",True)
    _preorder_admin_state[c.from_user.id]={"action":"fulfill","preorder_id":str(rid)}
    msg=raw_bot.send_message(c.from_user.id,"✅ FULFILL PREORDER\n\nSend the delivery/account/code/link/message that should be delivered to the customer.\n\n/cancel to stop.")
    bot.register_next_step_handler(msg,preorder_admin_fulfill_receive); bot.answer_callback_query(c.id,"Send delivery")

def preorder_admin_fulfill_receive(m):
    st=_preorder_admin_state.get(m.from_user.id); text=(m.text or "").strip()
    if not st or st.get("action")!="fulfill": return
    if text.lower()=="/cancel": _preorder_admin_state.pop(m.from_user.id,None); return raw_bot.send_message(m.chat.id,"❌ Fulfillment cancelled.",reply_markup=admin_menu())
    try: rid=ObjectId(st["preorder_id"])
    except Exception: _preorder_admin_state.pop(m.from_user.id,None); return
    r=preorders_col.find_one({"_id":rid,"status":"pending_fulfillment"})
    if not r: _preorder_admin_state.pop(m.from_user.id,None); return raw_bot.send_message(m.chat.id,"❌ Preorder was already processed.")
    upd=preorders_col.update_one({"_id":rid,"status":"pending_fulfillment"},{"$set":{"status":"fulfilled","fulfilled_at":time.time(),"fulfilled_by":m.from_user.id,"delivery":text[:10000]}})
    if upd.modified_count!=1: return raw_bot.send_message(m.chat.id,"❌ Could not claim preorder.")
    _preorder_admin_state.pop(m.from_user.id,None)
    try: raw_bot.send_message(int(r["user_id"]),f"✅ YOUR PREORDER IS READY\n\nProduct: {r.get('product_name')}\nPreorder: #{str(r['_id'])[-8:].upper()}\n\n{text[:3500]}\n\nThank you for trusting GLOBEXOMART. 👑")
    except Exception: pass
    raw_bot.send_message(m.chat.id,"✅ Preorder fulfilled and sent to the customer.",reply_markup=admin_menu())

@bot.callback_query_handler(func=lambda c: c.data.startswith("preorder_refundask|"))
def preorder_refund_ask(c):
    if not is_admin(c.from_user.id): return bot.answer_callback_query(c.id,"Admin only",True)
    rid=c.data.split("|",1)[1]; kb=InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("✅ Refund to User Balance",callback_data=f"preorder_refund|{rid}"),InlineKeyboardButton("❌ Cancel",callback_data="preorder_admin_list"))
    raw_bot.send_message(c.from_user.id,"↩️ Refund this paid preorder? The full prepaid amount will be returned to the user's USDT bot balance.",reply_markup=kb); bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("preorder_refund|"))
def preorder_refund(c):
    if not is_admin(c.from_user.id): return bot.answer_callback_query(c.id,"Admin only",True)
    try: rid=ObjectId(c.data.split("|",1)[1])
    except Exception: return bot.answer_callback_query(c.id,"Invalid",True)
    r=preorders_col.find_one({"_id":rid,"status":"pending_fulfillment","payment_status":"approved"})
    if not r: return bot.answer_callback_query(c.id,"Already processed or missing",True)
    amt=float(r.get("total",0) or 0); uid=int(r.get("user_id"))
    upd=preorders_col.update_one({"_id":rid,"status":"pending_fulfillment"},{"$set":{"status":"refunded","payment_status":"refunded","refunded_at":time.time(),"refunded_by":c.from_user.id}})
    if upd.modified_count!=1: return bot.answer_callback_query(c.id,"Could not process",True)
    users_col.update_one({"_id":str(uid)},{"$inc":{"usdt_balance":amt}})
    try: raw_bot.send_message(uid,f"↩️ PREORDER REFUNDED\n\nProduct: {r.get('product_name')}\nAmount returned: {_shop_money(amt)}\nThe amount has been credited back to your bot USDT balance.")
    except Exception: pass
    bot.answer_callback_query(c.id,"Refunded",True); raw_bot.send_message(c.from_user.id,f"✅ Refunded {_shop_money(amt)} to user {uid}.",reply_markup=admin_menu())

# =========================
# ✨ V32 GLOBAL PREMIUM EMOJI LAYER
# =========================
# These packs were supplied by the owner. The bot imports them at startup and
# uses their custom_emoji_id values across generated messages wherever Telegram
# allows entities without breaking existing Markdown/HTML formatting.
DEFAULT_PREMIUM_EMOJI_PACK_LINKS = [
    "https://t.me/addemoji/getmodpc",
    "https://t.me/addemoji/ZEROX_660_by_TgEmodziBot",
    "https://t.me/addemoji/Anonymous_Anaconda_by_fStikBot",
    "https://t.me/addemoji/Winy_xEmoji_pack_by_fStikBot",
    "https://t.me/addemoji/BirthdayCollection",
    "https://t.me/addemoji/Taj_Mehyar",
    "https://t.me/addemoji/FinanceEmoji",
    "https://t.me/addemoji/TONEmoji",
]

# Keep originals so the global renderer can retry safely without recursion.
_V32_RAW_SEND_MESSAGE = raw_bot.send_message
_V32_RAW_EDIT_MESSAGE_TEXT = raw_bot.edit_message_text
_V32_BOT_SEND_MESSAGE = bot.send_message


def _v32_emoji_enabled():
    try:
        return bool(get_cached_config().get("global_premium_emojis_enabled", True))
    except Exception:
        return True


def _v32_emoji_intensity():
    try:
        val = str(get_cached_config().get("premium_emoji_intensity", "max") or "max").lower()
        return val if val in ("off", "normal", "max") else "max"
    except Exception:
        return "max"


def _v32_contains_emoji(text):
    # Practical broad check for emoji/symbol ranges used throughout the bot.
    return bool(re.search(r"[\U0001F000-\U0001FAFF\u2600-\u27BF]", str(text or "")))


def _v32_context_prefix(text):
    """Add useful emoji to plain generated messages in MAX mode.

    This never removes/replaces user content and only runs when a generated
    message contains no emoji at all. It keeps the UI lively without stacking
    duplicate icons on already-decorated messages.
    """
    t = str(text or "").lower()
    rules = [
        (("error", "failed", "invalid", "wrong", "reject", "denied"), "❌"),
        (("warning", "attention", "risk", "suspicious"), "⚠️"),
        (("success", "approved", "complete", "saved", "added", "updated", "done"), "✅"),
        (("vip", "premium", "subscription"), "💎"),
        (("payment", "deposit", "withdraw", "paid", "price", "usdt", "balance", "revenue"), "💰"),
        (("product", "order", "preorder", "stock", "delivery", "cart"), "📦"),
        (("method", "guide", "document", "learn"), "📚"),
        (("referral", "refer", "affiliate", "commission", "earn"), "🎁"),
        (("scanner", "search", "find"), "🔎"),
        (("support", "ticket", "chat", "reply"), "💬"),
        (("deal", "discount", "offer", "sale", "trending"), "🔥"),
        (("invoice", "proof", "receipt"), "🧾"),
...
def _v32_prepare_text(text):
    text = str(text or "")
    if not _v32_emoji_enabled() or _v32_emoji_intensity() == "off":
        return text
    if _v32_emoji_intensity() == "max" and text.strip() and not _v32_contains_emoji(text):
        return f"{_v32_context_prefix(text)} {text}"
    return text


def _v32_premium_entities(text):
    if not _v32_emoji_enabled() or _v32_emoji_intensity() == "off":
        return None
    return _premium_entities_for_text(text)


def _v32_raw_send_message(chat_id, text, *args, **kwargs):
    text = _v32_prepare_text(text)
    # Do not overwrite explicitly supplied entities. Do not mix custom entities
    # with Markdown/HTML parsing; Telegram treats those as separate formatting
    # mechanisms. Those messages still keep their normal Unicode emoji.
    explicit_parse = kwargs.get("parse_mode")
    if not kwargs.get("entities") and not explicit_parse:
        ents = _v32_premium_entities(text)
        if ents:
            k = dict(kwargs)
            k["entities"] = ents
            try:
                return _V32_RAW_SEND_MESSAGE(chat_id, text, *args, **k)
            except Exception as exc:
                log_event("global_premium_emoji_fallback", target=chat_id, details={"error": str(exc)[:500]}, level="warning")
    return _V32_RAW_SEND_MESSAGE(chat_id, text, *args, **kwargs)


def _v32_raw_edit_message_text(text, *args, **kwargs):
    text = _v32_prepare_text(text)
    explicit_parse = kwargs.get("parse_mode")
    if not kwargs.get("entities") and not explicit_parse:
        ents = _v32_premium_entities(text)
        if ents:
            k = dict(kwargs); k["entities"] = ents
            try:
                return _V32_RAW_EDIT_MESSAGE_TEXT(text, *args, **k)
            except Exception:
                pass
    return _V32_RAW_EDIT_MESSAGE_TEXT(text, *args, **kwargs)


def _v32_bot_send_message(chat_id, text, *args, **kwargs):
    text = _v32_prepare_text(text)
    # bot has a default Markdown parse mode, therefore only inject custom emoji
    # entities when the caller explicitly disables parsing. Otherwise keep the
    # premium-looking Unicode decoration and preserve Markdown formatting.
    if kwargs.get("parse_mode", "__default__") is None and not kwargs.get("entities"):
        ents = _v32_premium_entities(text)
        if ents:
            k = dict(kwargs); k["entities"] = ents
            try:
                return _V32_BOT_SEND_MESSAGE(chat_id, text, *args, **k)
            except Exception:
                pass
    return _V32_BOT_SEND_MESSAGE(chat_id, text, *args, **kwargs)


def _premium_send_text(chat_id, text, **kwargs):
    """V32-safe premium sender using the original raw send method directly."""
    text = _v32_prepare_text(text)
    kwargs.pop("parse_mode", None)
    ents = _v32_premium_entities(text)
    if ents:
        k = dict(kwargs); k["entities"] = ents
        try:
            return _V32_RAW_SEND_MESSAGE(chat_id, text, **k)
        except Exception as exc:
            log_event("premium_emoji_send_fallback", target=chat_id, details={"error": str(exc)[:500]}, level="warning")
    return _V32_RAW_SEND_MESSAGE(chat_id, text, **kwargs)


def _import_default_premium_emoji_packs(force=False):
    """Import/update the owner's default packs and return (mapping_count, failed)."""
    try:
        cfg = get_config()
        saved_links = list(cfg.get("premium_emoji_pack_links") or [])
        merged_links = list(dict.fromkeys(saved_links + DEFAULT_PREMIUM_EMOJI_PACK_LINKS))
        if merged_links != saved_links:
            set_config("premium_emoji_pack_links", merged_links)
        if not force and cfg.get("default_premium_packs_imported") and _premium_emoji_map():
            return 0, []
        mapping = _premium_emoji_map()
        before = dict(mapping)
        failed = []
        for link in DEFAULT_PREMIUM_EMOJI_PACK_LINKS:
            m = re.search(r"t\.me/addemoji/([A-Za-z0-9_]+)", link, re.I)
            if not m:
                continue
            pack_name = m.group(1)
            try:
        ...
        if mapping != before:
            set_config("premium_emoji_map", mapping)
        if not failed:
            set_config("default_premium_packs_imported", True)
        return max(0, len(mapping) - len(before)), failed
    except Exception as exc:
        log_event("default_premium_pack_import_error", details={"error": str(exc)[:1000]}, level="error")
        return 0, ["startup_error"]


# Enable global rendering for all raw generated messages. Existing copied/
# forwarded Telegram messages already preserve their custom emoji entities.
raw_bot.send_message = _v32_raw_send_message
raw_bot.edit_message_text = _v32_raw_edit_message_text
bot.send_message = _v32_bot_send_message


# Import owner packs once before long polling starts. Failure never blocks bot.
_V32_RUN_BOT_ORIGINAL = run_bot
def run_bot():
    try:
        added, failed = _import_default_premium_emoji_packs(force=False)
        print(f"✨ Premium emoji startup: +{added} mappings, {len(failed)} packs failed", flush=True)
    except Exception as exc:
        print(f"⚠️ Premium emoji startup skipped: {exc}", flush=True)
    return _V32_RUN_BOT_ORIGINAL()




# =========================
# 🚀 V34 CUSTOMER EXPERIENCE + RELIABILITY
# =========================
# New user-facing tools are members of MAIN_MENU_BUTTONS but are placed in
# DEFAULT_MORE_BUTTONS above, so admins can pull them out through the existing
# Arrange Main Buttons -> Manage More Menu screen without special-case code.

v34_deliveries_col = db["deliveries_v34"]
v34_warranty_col = db["warranty_requests_v34"]
v34_announcements_col = db["announcements_v34"]
v34_failed_jobs_col = db["failed_jobs_v34"]
v34_menu_drafts_col = db["menu_drafts_v34"]
v34_config_history_col = db["config_history_v34"]
for _c, _idx in [
    (v34_deliveries_col, [("user_id",1),("created_at",-1)]),
    (v34_warranty_col, [("user_id",1),("status",1),("created_at",-1)]),
    (v34_announcements_col, [("active",1),("created_at",-1)]),
    (v34_failed_jobs_col, [("status",1),("created_at",-1)]),
    (v34_config_history_col, [("created_at",-1)]),
]:
    try: _c.create_index(_idx)
    except Exception: pass


def _v34_fmt_ts(value):
    try:
...
def _v34_user_orders(uid, limit=15):
    """Best-effort unified read across existing order/payment collections."""
    out=[]
    for col, kind in [(payments_col,"Payment"),(db["orders"],"Product"),(db["preorders"],"Preorder")]:
        try:
            rows=list(col.find({"user_id":int(uid)}).sort("created_at",-1).limit(limit))
            for r in rows:
                r=dict(r); r["_v34_kind"]=kind; out.append(r)
        except Exception: pass
    out.sort(key=lambda r: float(r.get("created_at",r.get("paid_at",0)) or 0), reverse=True)
    return out[:limit]


def _v34_order_name(r):
    return str(r.get("product_name") or r.get("method_name") or r.get("plan_name") or r.get("item_name") or r.get("title") or r.get("type") or r.get("_v34_kind") or "Order")


@bot.message_handler(func=lambda m: m.text == "📍 Order Tracker")
def v34_order_tracker(m):
    rows=_v34_user_orders(m.from_user.id,10)
    if not rows: return raw_bot.send_message(m.chat.id,"📍 ORDER TRACKER\n\nNo orders found yet.",reply_markup=more_menu(m.from_user.id))
    lines=["📍 ORDER TRACKER","","Your latest orders:"]
    for r in rows:
        status=str(r.get("status") or "submitted").replace("_"," ").title()
        oid=str(r.get("order_id") or r.get("invoice_id") or r.get("_id"))[-8:]
        lines.append(f"\n📦 {_v34_order_name(r)}\n🆔 {oid}\n🚦 {status}\n🕒 {_v34_fmt_ts(r.get('updated_at') or r.get('created_at') or r.get('paid_at'))}")
    raw_bot.send_message(m.chat.id,"\n".join(lines)[:3900],reply_markup=more_menu(m.from_user.id))


@bot.message_handler(func=lambda m: m.text == "📦 Delivery Center")
def v34_delivery_center(m):
    rows=list(v34_deliveries_col.find({"user_id":int(m.from_user.id)}).sort("created_at",-1).limit(20))
    if not rows: return raw_bot.send_message(m.chat.id,"📦 DELIVERY CENTER\n\nNo saved deliveries yet. Delivered products/files/codes can appear here.",reply_markup=more_menu(m.from_user.id))
    lines=["📦 DELIVERY CENTER",""]
    for r in rows:
        lines.append(f"✅ {r.get('name','Delivery')}\n🧾 {r.get('order_id','—')}\n🕒 {_v34_fmt_ts(r.get('created_at'))}\n📬 {str(r.get('delivery',''))[:500]}")
    raw_bot.send_message(m.chat.id,"\n\n".join(lines)[:3900],reply_markup=more_menu(m.from_user.id))


@bot.message_handler(func=lambda m: m.text == "💎 Renewal Center")
def v34_renewal_center(m):
    u=users_col.find_one({"_id":int(m.from_user.id)}) or users_col.find_one({"user_id":int(m.from_user.id)}) or {}
    exp=u.get("vip_expiry") or u.get("vip_until") or u.get("vip_expires_at")
    txt="💎 VIP RENEWAL CENTER\n\n"
    if exp: txt+=f"👑 Current VIP expiry: {_v34_fmt_ts(exp)}\n\n"
    else: txt+="👤 No active VIP expiry was found on your profile.\n\n"
    txt+="⭐ Use Buy VIP to renew or upgrade using the currently available plans. Renewal reminders can be sent before expiry."
    raw_bot.send_message(m.chat.id,txt,reply_markup=more_menu(m.from_user.id))


@bot.message_handler(func=lambda m: m.text == "🔔 Expiry Center")
def v34_expiry_center(m):
    uid=int(m.from_user.id); lines=["🔔 EXPIRY CENTER",""]
    u=users_col.find_one({"_id":uid}) or users_col.find_one({"user_id":uid}) or {}
    exp=u.get("vip_expiry") or u.get("vip_until") or u.get("vip_expires_at")
    lines.append(f"💎 VIP: {_v34_fmt_ts(exp) if exp else 'No active expiry found'}")
    try:
        pres=list(db["preorders"].find({"user_id":uid,"status":{"$in":["paid","pending","approved"]}}).sort("created_at",-1).limit(5))
        for r in pres: lines.append(f"📦 {_v34_order_name(r)} ETA: {r.get('expected_delivery') or r.get('eta') or 'Awaiting admin update'}")
    except Exception: pass
    raw_bot.send_message(m.chat.id,"\n".join(lines)[:3900],reply_markup=more_menu(uid))


@bot.message_handler(func=lambda m: m.text == "🛡 Warranty & Replacement")
def v34_warranty_start(m):
    rows=_v34_user_orders(m.from_user.id,12)
    kb=InlineKeyboardMarkup(row_width=1)
    for r in rows[:10]:
        oid=str(r.get("order_id") or r.get("_id"))
        kb.add(InlineKeyboardButton(f"📦 {_v34_order_name(r)[:32]}",callback_data=f"v34war|new|{oid[-24:]}"))
    kb.add(InlineKeyboardButton("📋 My Requests",callback_data="v34war|mine"))
    raw_bot.send_message(m.chat.id,"🛡 WARRANTY & REPLACEMENT\n\nChoose an order to request help/replacement. Admin will review warranty eligibility and your evidence.",reply_markup=kb)


@bot.callback_query_handler(func=lambda c: str(c.data or "").startswith("v34war|"))
def v34_warranty_cb(c):
    parts=c.data.split("|"); action=parts[1]
    if action=="mine":
        rows=list(v34_warranty_col.find({"user_id":int(c.from_user.id)}).sort("created_at",-1).limit(15))
        txt="🛡 MY WARRANTY REQUESTS\n\n"+("\n".join(f"• {r.get('order_ref','—')} — {str(r.get('status','pending')).title()}" for r in rows) if rows else "No requests yet.")
        bot.answer_callback_query(c.id); return raw_bot.send_message(c.from_user.id,txt)
    ref=parts[2]
    msg=raw_bot.send_message(c.from_user.id,f"🛡 WARRANTY REQUEST\n\nOrder reference: {ref}\n\nDescribe the problem. You can send text or a photo with caption.")
    bot.register_next_step_handler(msg,v34_warranty_submit,ref)
    bot.answer_callback_query(c.id)


def v34_warranty_submit(m,ref):
    doc={"user_id":int(m.from_user.id),"username":m.from_user.username,"order_ref":str(ref),"status":"pending","created_at":time.time(),"text":(m.text or m.caption or "")[:3000]}
    if getattr(m,"photo",None): doc["photo_file_id"]=m.photo[-1].file_id
    v34_warranty_col.insert_one(doc)
    raw_bot.send_message(m.chat.id,"✅ Warranty/replacement request submitted. Admin will review it.",reply_markup=more_menu(m.from_user.id))


@bot.message_handler(func=lambda m: m.text == "🎁 Reward Center")
def v34_reward_center(m):
    uid=int(m.from_user.id); u=users_col.find_one({"_id":uid}) or users_col.find_one({"user_id":uid}) or {}
    raw_bot.send_message(m.chat.id,f"🎁 REWARD CENTER\n\n💰 Points: {u.get('points',0)}\n💵 Cashback: ${float(u.get('cashback',0) or 0):.2f}\n🎁 Referrals: {u.get('referrals',u.get('ref_count',0))}\n🏅 Loyalty: {u.get('loyalty_level','Standard')}\n\nUse the existing referral, redeem, loyalty and daily reward tools for available rewards.",reply_markup=more_menu(uid))


@bot.message_handler(func=lambda m: m.text == "📣 Announcements")
def v34_announcements(m):
    rows=list(v34_announcements_col.find({"active":{"$ne":False}}).sort("created_at",-1).limit(10))
    txt="📣 ANNOUNCEMENTS\n\n"+("\n\n".join(f"📌 {r.get('title','Update')}\n{r.get('text','')}" for r in rows) if rows else "No active announcements right now.")
    raw_bot.send_message(m.chat.id,txt[:3900],reply_markup=more_menu(m.from_user.id))


@bot.message_handler(func=lambda m: m.text == "💰 Earnings Dashboard")
def v34_earnings_dashboard(m):
    uid=int(m.from_user.id); u=users_col.find_one({"_id":uid}) or users_col.find_one({"user_id":uid}) or {}
    raw_bot.send_message(m.chat.id,f"💰 EARNINGS DASHBOARD\n\n👥 Qualified referrals: {u.get('referrals',u.get('ref_count',0))}\n💵 Available earnings: ${float(u.get('ref_earnings',u.get('affiliate_balance',0)) or 0):.2f}\n⏳ Pending commission: ${float(u.get('pending_commission',0) or 0):.2f}\n✅ Total paid: ${float(u.get('commission_paid',0) or 0):.2f}\n\nOpen 🎁 Referral / 💼 Earn for your links and current earning rules.",reply_markup=more_menu(uid))


# ---- V34 admin operations ----
def _v34_admin_hub_markup():
    kb=InlineKeyboardMarkup(row_width=2)
    kb.row(InlineKeyboardButton("🛡 Warranty Queue",callback_data="v34adm|warranty"),InlineKeyboardButton("📣 Announcements",callback_data="v34adm|announce"))
    kb.row(InlineKeyboardButton("🧪 Preview User Menus",callback_data="v34adm|preview"),InlineKeyboardButton("↩️ Menu Draft/Publish",callback_data="v34adm|menudraft"))
    kb.row(InlineKeyboardButton("🚨 Failed Jobs",callback_data="v34adm|failed"),InlineKeyboardButton("🧾 Config History",callback_data="v34adm|history"))
    kb.row(InlineKeyboardButton("📊 Conversion",callback_data="v34adm|conversion"),InlineKeyboardButton("💰 Profit Snapshot",callback_data="v34adm|profit"))
    return kb

@bot.message_handler(func=lambda m: m.text == "🚀 V34 Operations" and is_admin(m.from_user.id))
def v34_admin_hub(m): raw_bot.send_message(m.chat.id,"🚀 V34 OPERATIONS\n\nCustomer experience, menu publishing, warranty, reliability and reporting tools.",reply_markup=_v34_admin_hub_markup())

@bot.callback_query_handler(func=lambda c: str(c.data or "").startswith("v34adm|") and is_admin(c.from_user.id))
def v34_admin_cb(c):
    a=c.data.split("|",1)[1]; bot.answer_callback_query(c.id)
    if a=="warranty":
        rows=list(v34_warranty_col.find({"status":"pending"}).sort("created_at",1).limit(30))
        txt="🛡 PENDING WARRANTY / REPLACEMENT\n\n"+("\n".join(f"• @{r.get('username') or r.get('user_id')} | {r.get('order_ref')} | {r.get('text','')[:80]}" for r in rows) if rows else "No pending requests.")
    elif a=="announce":
        msg=raw_bot.send_message(c.from_user.id,"📣 Send announcement as:\nTitle | Message")
        bot.register_next_step_handler(msg,v34_announcement_save); return
    elif a=="preview":
        txt="🧪 USER MENU PREVIEW\n\nNew V34 user tools are inside 🧰 More by default. Use ↕️ Arrange Main Buttons → 🧰 Manage More Menu to pull any individual tool onto the main menu.\n\nThis uses the same visibility rules as the real menu."
    elif a=="menudraft":
        txt="↩️ MENU DRAFT / PUBLISH\n\nUse ↕️ Arrange Main Buttons to build rows (1–3 buttons each). V34 keeps the new tools in More by default; Manage More Menu can move each one in/out independently."
    elif a=="failed":
        n=v34_failed_jobs_col.count_documents({"status":{"$ne":"resolved"}}); txt=f"🚨 FAILED JOB RETRY QUEUE\n\nUnresolved jobs: {n}\n\nFailed background actions can be recorded here and retried safely by job-specific handlers."
    elif a=="history":
        rows=list(v34_config_history_col.find().sort("created_at",-1).limit(15)); txt="🧾 CONFIG HISTORY\n\n"+("\n".join(f"• {_v34_fmt_ts(r.get('created_at'))} — {r.get('key','setting')}" for r in rows) if rows else "No V34 configuration changes recorded yet.")
    elif a=="conversion":
        try: pay=payments_col.count_documents({}); approved=payments_col.count_documents({"status":{"$in":["approved","paid"]}})
        except Exception: pay=approved=0
        txt=f"📊 CONVERSION SNAPSHOT\n\n📸 Payment/proof records: {pay}\n✅ Approved/paid: {approved}\n📈 Approval rate: {(approved/pay*100 if pay else 0):.1f}%"
    else:
        txt="💰 PROFIT SNAPSHOT\n\nUse the existing Financial Ledger / Profit Tracking for revenue, costs, refunds, cashback, commissions and expenses. V34 keeps this view consolidated rather than duplicating accounting records."
    raw_bot.send_message(c.from_user.id,txt[:3900])


def v34_announcement_save(m):
    raw=(m.text or "").strip()
    if "|" not in raw: return raw_bot.send_message(m.chat.id,"❌ Use: Title | Message")
    title,body=[x.strip() for x in raw.split("|",1)]
    v34_announcements_col.insert_one({"title":title[:100],"text":body[:3500],"active":True,"created_at":time.time(),"created_by":int(m.from_user.id)})
    raw_bot.send_message(m.chat.id,"✅ Announcement saved. Users can read it from 📣 Announcements.",reply_markup=admin_menu())

# Add one consolidated admin entry; user-facing V34 tools stay in More by default.
_v34_admin_menu_original = admin_menu
def admin_menu():
    kb=_v34_admin_menu_original()
    try: kb.row("🚀 V34 Operations")
    except Exception: pass
    return kb

BOT_BUILD_VERSION='35.0'


# =========================
# 🎁 V36 GIFT LOGIN LINKS
# =========================
_gift_user_locks={}
_gift_user_locks_guard=threading.Lock()


def _gift_lock(uid):
    uid=int(uid)
    with _gift_user_locks_guard:
        lock=_gift_user_locks.get(uid)
        if lock is None:
            lock=threading.RLock(); _gift_user_locks[uid]=lock
        return lock


def _gift_button_name():
    value=str(get_cached_config().get("gift_button_name","🎁 Gift") or "🎁 Gift")
    value=" ".join(value.replace("\n"," ").split())[:40]
    return value or "🎁 Gift"


def _gift_report_limit():
    try: return max(1,min(100,int(get_cached_config().get("gift_bad_report_limit",5) or 5)))
    except Exception: return 5


def _gift_kind_label(kind):
    return "Phone Login URL" if kind=="mobile" else "PC Login URL"


def _gift_extract_urls(text):
    found=[]; seen=set()
    for raw in re.findall(r"https?://[^\s<>\"']+",str(text or ""),flags=re.I):
        ...
        try:
            ...
            continue
        if len(url)>2000 or url in seen: continue
        seen.add(url); found.append(url)
    return found


def _gift_extract_typed_urls(text):
    """Split one mixed TXT into phone, PC and unclassified URL lists."""
    groups={"mobile":[],"pc":[],"unclassified":[]}
    seen={key:set() for key in groups}
    mobile_label=re.compile(
        r"\b(?:phone|mobile|android|ios)\s*(?:login\s*)?(?:urls?|links?|buttons?)?\b",
        re.I,
    )
    pc_label=re.compile(
        r"\b(?:pc|desktop|computer|windows|mac(?:os)?)\s*(?:login\s*)?(?:urls?|links?|buttons?)?\b",
        re.I,
    )
    ...
    def add(kind,url):
        normalized=_gift_extract_urls(url)
        if not normalized: return
        url=normalized[0]
        if url not in seen[kind]:
            seen[kind].add(url); groups[kind].append(url)

    for line in str(text or "").splitlines():
        mobile_marks=[(m.start(),m.end(),"mobile") for m in mobile_label.finditer(line)]
        pc_marks=[(m.start(),m.end(),"pc") for m in pc_label.finditer(line)]
        marks=sorted(mobile_marks+pc_marks,key=lambda item:item[0])
        url_marks=list(url_pattern.finditer(line))

        # A heading/label on its own applies to the URL lines that follow it.
        if not url_marks:
            kinds={item[2] for item in marks}
            if len(kinds)==1: current_kind=next(iter(kinds))
            continue

        for url_match in url_marks:
            before=[item for item in marks if item[0]<url_match.start()]
            if before:
                kind=before[-1][2]
            else:
                kinds={item[2] for item in marks}
                if len(kinds)==1:
                    ...
                    lower=url_match.group(0).lower()
                    if any(token in lower for token in ("mobile","phone","android","ios")):
                        kind="mobile"
                    elif any(token in lower for token in ("desktop","computer","windows","macos","/pc","pc-","pc_")):
                        kind="pc"
                    else:
                        kind="unclassified"
            add(kind,url_match.group(0))

        # The final label on a mixed line becomes the context for following lines.
        if marks: current_kind=marks[-1][2]
    return groups


def _gift_import_urls(kind,urls,admin_id):
    if kind not in {"mobile","pc"}: raise ValueError("Invalid login-link type")
    added=duplicates=0; now=time.time()
    for url in urls:
        result=gift_links_col.update_one(
            {"kind":kind,"url":url},
            {"$setOnInsert":{
                "kind":kind,"url":url,"status":"active","report_count":0,"delivery_count":0,
                "created_at":now,"created_by":int(admin_id),
            }},
            upsert=True,
        )
...
def _gift_next_link(uid,kind):
    if kind not in {"mobile","pc"}: return None
    limit=_gift_report_limit()
    seen=[row.get("link_id") for row in gift_link_deliveries_col.find(
        {"user_id":int(uid),"kind":kind},{"link_id":1}
    ) if row.get("link_id") is not None]
    query={"kind":kind,"status":"active","report_count":{"$lt":limit}}
    if seen: query["_id"]={"$nin":seen}
    row=gift_links_col.find_one_and_update(
        query,
        {"$inc":{"delivery_count":1},"$set":{"last_delivered_at":time.time()}},
        sort=[("delivery_count",1),("created_at",1)],
        return_document=ReturnDocument.AFTER,
    )
    if not row: return None
    try:
        gift_link_deliveries_col.insert_one({
            "link_id":row["_id"],"user_id":int(uid),"kind":kind,
            "created_at":time.time(),"reported_not_working":False,
        })
    except Exception:
        pass
    return row
...
def _gift_price_config():
    cfg=get_cached_config()
    currency=str(cfg.get("gift_price_currency","points") or "points").lower()
    if currency not in {"points","usdt"}: currency="points"
    try: amount=max(0.0,float(cfg.get("gift_price_amount",0) or 0))
    except Exception: amount=0.0
    if not math.isfinite(amount): amount=0.0
    if currency=="points": amount=int(amount)
    else: amount=round(amount,2)
    return currency,amount


def _gift_price_text():
    currency,amount=_gift_price_config()
    if amount<=0: return "Free"
    return f"{int(amount):,} points" if currency=="points" else f"${amount:g} USDT"


def _gift_user_has_access(uid):
    if is_admin(uid): return True
    _,amount=_gift_price_config()
    if amount<=0: return True
    row=users_col.find_one({"_id":str(int(uid))},{"gift_access_unlocked":1}) or {}
    if row.get("gift_access_unlocked"): return True
    return gift_access_purchases_col.find_one({"user_id":int(uid)},{"_id":1}) is not None


def _gift_show_purchase(uid):
    uid=int(uid); currency,amount=_gift_price_config()
    if amount<=0 or _gift_user_has_access(uid): return _gift_show_login_menu(uid)
    row=users_col.find_one({"_id":str(uid)},{"points":1,"usdt_balance":1}) or {}
    if currency=="points":
        balance=f"{int(row.get('points',0) or 0):,} points"
    else:
        balance=f"${float(row.get('usdt_balance',0) or 0):.2f} USDT"
    kb=InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton(f"🔓 Unlock for {_gift_price_text()}",callback_data="giftbuy|confirm"))
    raw_bot.send_message(
        uid,
        f"🎁 GIFT ACCESS\n\nOne-time access price: {_gift_price_text()}\nYour balance: {balance}\n\nAfter purchase, the Gift folder remains unlocked for your account.",
        reply_markup=kb,
    )


def _gift_show_login_menu(uid):
    uid=int(uid); limit=_gift_report_limit()
    mobile=gift_links_col.count_documents({"kind":"mobile","status":"active","report_count":{"$lt":limit}})
    pc=gift_links_col.count_documents({"kind":"pc","status":"active","report_count":{"$lt":limit}})
    kb=InlineKeyboardMarkup(row_width=2)
    kb.row(
        InlineKeyboardButton("📱 Mobile Login",callback_data="giftopen|mobile"),
        InlineKeyboardButton("💻 PC Login",callback_data="giftopen|pc"),
    )
    raw_bot.send_message(
        uid,
        f"🎁 {_gift_button_name().replace('🎁','').strip() or 'GIFT'}\n\nChoose your login type below.\n\n📱 Mobile links available: {mobile}\n💻 PC links available: {pc}",
        reply_markup=kb,
    )


def _gift_send_link(uid,kind,replacement=False):
    uid=int(uid)
    if not _gift_user_has_access(uid): return _gift_show_purchase(uid)
    with _gift_lock(uid):
        row=_gift_next_link(uid,kind)
        if not row:
            prefix="No more replacement links are available" if replacement else "No working links are available"
            return raw_bot.send_message(
                uid,
                f"⏳ {prefix} for {_gift_kind_label(kind)} right now. Please try again later.",
                reply_markup=main_menu(uid),
            )
        kb=InlineKeyboardMarkup(row_width=1)
        kb.add(InlineKeyboardButton(f"🔗 {_gift_kind_label(kind)}",url=row["url"]))
        kb.add(InlineKeyboardButton("❌ Not Working",callback_data=f"giftbad|{row['_id']}|{kind}"))
        heading="NEW REPLACEMENT LINK" if replacement else "YOUR LOGIN LINK"
        return raw_bot.send_message(
            uid,
            f"🎁 {heading}\n\nTap the {_gift_kind_label(kind)} button below. If the page does not work, tap Not Working and the bot will automatically send a different link.",
            reply_markup=kb,
            disable_web_page_preview=True,
        )


@bot.message_handler(func=lambda m:str(getattr(m,"text","") or "") in {_gift_button_name(),"🎁 Gift"})
@force_join_handler
def gift_login_menu(m):
    if _gift_user_has_access(m.from_user.id): return _gift_show_login_menu(m.from_user.id)
    return _gift_show_purchase(m.from_user.id)


@bot.callback_query_handler(func=lambda c:str(c.data or "")=="giftbuy|confirm")
def gift_purchase_confirm_cb(c):
    uid=int(c.from_user.id)
    if force_block(uid): return
    with _gift_lock(uid):
        if _gift_user_has_access(uid):
            bot.answer_callback_query(c.id,"Gift access is already unlocked",True)
            return _gift_show_login_menu(uid)
        currency,amount=_gift_price_config()
        if amount<=0:
            bot.answer_callback_query(c.id,"Gift access is free",True)
            return _gift_show_login_menu(uid)
        now=time.time()
        query={"_id":str(uid),"gift_access_unlocked":{"$ne":True}}
        if currency=="points":
            charge=int(amount); query["points"]={"$gte":charge}
            update={"$inc":{"points":-charge,"total_points_spent":charge},"$set":{"gift_access_unlocked":True,"gift_access_unlocked_at":now,"gift_access_currency":"points","gift_access_price":charge}}
        else:
            charge=round(float(amount),2); query["usdt_balance"]={"$gte":charge}
            update={"$inc":{"usdt_balance":-charge},"$set":{"gift_access_unlocked":True,"gift_access_unlocked_at":now,"gift_access_currency":"usdt","gift_access_price":charge}}
        updated=users_col.find_one_and_update(query,update,return_document=ReturnDocument.AFTER)
        if not updated:
            row=users_col.find_one({"_id":str(uid)},{"points":1,"usdt_balance":1}) or {}
            if currency=="points": message=f"Not enough points. Required: {int(charge):,}; balance: {int(row.get('points',0) or 0):,}."
            else: message=f"Insufficient USDT balance. Required: ${charge:g}; balance: ${float(row.get('usdt_balance',0) or 0):.2f}. Use Deposit first."
            bot.answer_callback_query(c.id,"Insufficient balance",True)
            return raw_bot.send_message(uid,"❌ "+message)
        gift_access_purchases_col.update_one(
            {"user_id":uid},
            {"$setOnInsert":{"user_id":uid,"currency":currency,"amount":charge,"created_at":now,"status":"completed"}},
            upsert=True,
        )
        User._cache.pop(uid,None); User._cache.pop(str(uid),None)
        User._cache_time.pop(uid,None); User._cache_time.pop(str(uid),None)
        try:
            if currency=="points": add_point_history(uid,-int(charge),"gift_folder_purchase")
            elif "_v27_ledger" in globals(): _v27_ledger("gift_folder_purchase",charge,user_id=uid,note="Gift folder access",direction="credit")
        except Exception: pass
        bot.answer_callback_query(c.id,"Gift access unlocked ✅",True)
        raw_bot.send_message(uid,f"✅ GIFT ACCESS UNLOCKED\n\nPaid: {_gift_price_text()}\nYour access is permanent.")
        return _gift_show_login_menu(uid)


@bot.callback_query_handler(func=lambda c:str(c.data or "").startswith("giftopen|"))
def gift_open_cb(c):
    kind=c.data.split("|",1)[1]
    if kind not in {"mobile","pc"}: return bot.answer_callback_query(c.id,"Invalid option",True)
    bot.answer_callback_query(c.id)
    _gift_send_link(c.from_user.id,kind)


@bot.callback_query_handler(func=lambda c:str(c.data or "").startswith("giftbad|"))
def gift_not_working_cb(c):
    try:
        _,oid_text,kind=c.data.split("|",2); oid=ObjectId(oid_text); uid=int(c.from_user.id)
        if kind not in {"mobile","pc"}: raise ValueError("Invalid link type")
        with _gift_lock(uid):
            delivery=gift_link_deliveries_col.find_one({"link_id":oid,"user_id":uid,"kind":kind})
            if not delivery:
                return bot.answer_callback_query(c.id,"This link was not assigned to you",True)
            if gift_link_reports_col.find_one({"link_id":oid,"user_id":uid},{"_id":1}):
                return bot.answer_callback_query(c.id,"You already reported this link",True)
            gift_link_reports_col.insert_one({"link_id":oid,"user_id":uid,"kind":kind,"created_at":time.time()})
            row=gift_links_col.find_one_and_update(
                {"_id":oid},
                {"$inc":{"report_count":1},"$set":{"last_reported_at":time.time()}},
                return_document=ReturnDocument.AFTER,
            )
            gift_link_deliveries_col.update_one(
                {"_id":delivery["_id"]},
                {"$set":{"reported_not_working":True,"reported_at":time.time()}},
            )
            if row and int(row.get("report_count",0) or 0)>=_gift_report_limit():
                gift_links_col.update_one(
                    {"_id":oid},
                    {"$set":{"status":"retired","retired_at":time.time(),"retired_reason":"not_working_reports"}},
                )
            bot.answer_callback_query(c.id,"Reported. Sending another link…",True)
            _gift_send_link(uid,kind,replacement=True)
    except Exception as exc:
        bot.answer_callback_query(c.id,("Could not report link: "+str(exc))[:180],True)


def _gift_admin_markup():
    kb=InlineKeyboardMarkup(row_width=2)
    kb.add(InlineKeyboardButton(f"💰 Gift Price: {_gift_price_text()}",callback_data="giftadm|price"))
    kb.add(InlineKeyboardButton("🔄 Import Phone + PC TXT",callback_data="giftadm|import|auto"))
    kb.row(
        InlineKeyboardButton("📱 Import Mobile Links",callback_data="giftadm|import|mobile"),
        InlineKeyboardButton("💻 Import PC Links",callback_data="giftadm|import|pc"),
    )
    kb.row(
        InlineKeyboardButton("📱 Manage Phone Links",callback_data="giftadm|list|mobile|0"),
        InlineKeyboardButton("💻 Manage PC Links",callback_data="giftadm|list|pc|0"),
    )
    kb.row(
        InlineKeyboardButton("🗑 Delete All Phone",callback_data="giftadm|purgekind|mobile"),
        InlineKeyboardButton("🗑 Delete All PC",callback_data="giftadm|purgekind|pc"),
    )
    kb.row(
        InlineKeyboardButton("✏️ Rename Gift Button",callback_data="giftadm|rename"),
        InlineKeyboardButton("📊 Link Statistics",callback_data="giftadm|stats"),
    )
    kb.add(InlineKeyboardButton("🚫 Recent Not-Working Reports",callback_data="giftadm|reports"))
    kb.add(InlineKeyboardButton("🧹 Delete Not-Working Links",callback_data="giftadm|purgebad"))
    return kb


@bot.message_handler(func=lambda m:m.text=="🎁 Gift Link Manager" and is_admin(m.from_user.id))
def gift_admin_menu(m):
    raw_bot.send_message(
        m.chat.id,
        f"🎁 GIFT LINK MANAGER\n\nCurrent access price: {_gift_price_text()}\n\nImport one mixed TXT containing both Phone Login URL and PC Login URL entries. The bot detects each label and places every link in the correct pool automatically. Separate imports are also available as a fallback.\n\nA link is retired after five different users report it as not working.",
        reply_markup=_gift_admin_markup(),
    )


@bot.callback_query_handler(func=lambda c:str(c.data or "").startswith("giftadm|") and is_admin(c.from_user.id))
def gift_admin_cb(c):
    parts=c.data.split("|"); action=parts[1] if len(parts)>1 else ""
    if action=="import" and len(parts)>2:
        kind=parts[2]
        if kind=="auto":
            prompt=("🔄 AUTO-IMPORT PHONE + PC LINKS\n\nSend one .txt file containing both types. "
                    "Use labels such as:\n\nPhone Login URL: https://example.com/phone\n"
                    "PC Login URL: https://example.com/pc\n\nThe bot will separate and import both pools automatically.")
        else:
            prompt=f"📥 IMPORT {_gift_kind_label(kind).upper()} LINKS\n\nSend a .txt file or paste all URLs in one message."
        msg=raw_bot.send_message(c.from_user.id,prompt)
        bot.register_next_step_handler(msg,gift_admin_import_message,kind)
    elif action=="rename":
        msg=raw_bot.send_message(c.from_user.id,f"Current main-menu name: {_gift_button_name()}\n\nSend the new Gift button name (maximum 40 characters):")
        bot.register_next_step_handler(msg,gift_admin_rename_save)
    elif action=="price":
        msg=raw_bot.send_message(
            c.from_user.id,
            f"💰 SET GIFT ACCESS PRICE\n\nCurrent: {_gift_price_text()}\n\nSend one of these formats:\npoints 500\nusdt 5\nfree\n\nUsers pay once and keep permanent Gift access.",
        )
        bot.register_next_step_handler(msg,gift_admin_price_save)
    elif action=="stats":
        gift_admin_stats(c.from_user.id)
    elif action=="reports":
        gift_admin_reports(c.from_user.id)
    elif action=="list" and len(parts)>2:
        kind=parts[2]; page=int(parts[3]) if len(parts)>3 and parts[3].isdigit() else 0
        gift_admin_links_list(c.from_user.id,kind,page)
    elif action=="purgebad":
        gift_admin_purge_prompt(c.from_user.id)
    elif action=="purgekind" and len(parts)>2:
        gift_admin_purge_kind_prompt(c.from_user.id,parts[2])
    bot.answer_callback_query(c.id)


def gift_admin_import_message(m,kind):
    try:
        if not is_admin(m.from_user.id): raise ValueError("Admin only")
        if getattr(m,"document",None):
            name=str(m.document.file_name or "").lower()
            if name and not name.endswith(".txt"): raise ValueError("Please send a .txt file")
            if int(getattr(m.document,"file_size",0) or 0)>5*1024*1024: raise ValueError("TXT file must be 5 MB or smaller")
            info=bot.get_file(m.document.file_id); data=bot.download_file(info.file_path)
            try: content=data.decode("utf-8-sig")
            except UnicodeDecodeError: content=data.decode("latin-1")
        else:
            content=(m.text or m.caption or "")
        if kind=="auto":
            groups=_gift_extract_typed_urls(content)
            phone_urls=groups["mobile"]; pc_urls=groups["pc"]; unknown=groups["unclassified"]
            if not phone_urls and not pc_urls:
                raise ValueError("No labeled Phone Login URL or PC Login URL entries were found")
            phone_added,phone_duplicates=_gift_import_urls("mobile",phone_urls,m.from_user.id)
            pc_added,pc_duplicates=_gift_import_urls("pc",pc_urls,m.from_user.id)
            raw_bot.send_message(
                m.chat.id,
                f"✅ MIXED TXT AUTO-IMPORT COMPLETE\n\n"
                f"📱 PHONE LINKS\nNew: {phone_added}\nAlready existed: {phone_duplicates}\nDetected: {len(phone_urls)}\n\n"
                f"💻 PC LINKS\nNew: {pc_added}\nAlready existed: {pc_duplicates}\nDetected: {len(pc_urls)}\n\n"
                f"Unlabeled URLs skipped: {len(unknown)}",
                reply_markup=admin_menu(),
            )
        else:
            urls=_gift_extract_urls(content)
            if not urls: raise ValueError("No valid http/https URLs were found")
            added,duplicates=_gift_import_urls(kind,urls,m.from_user.id)
            raw_bot.send_message(
                m.chat.id,
                f"✅ BULK IMPORT COMPLETE\n\nType: {_gift_kind_label(kind)}\nNew links: {added}\nAlready existed/skipped: {duplicates}\nTotal valid URLs read: {len(urls)}",
                reply_markup=admin_menu(),
            )
    except Exception as exc:
        admin_error(m.from_user.id,exc)
...
def gift_admin_rename_save(m):
    try:
        name=" ".join(str(m.text or "").replace("\n"," ").split())
        if not name: raise ValueError("Button name cannot be empty")
        if len(name)>40: raise ValueError("Button name must be 40 characters or shorter")
        if name in set(MAIN_MENU_BUTTONS)|{"⚙️ ADMIN PANEL","🎁 Gift Link Manager"}:
            raise ValueError("That name is already used by another menu button")
        set_config("gift_button_name",name)
        raw_bot.send_message(m.chat.id,f"✅ Gift main-menu button renamed to: {name}",reply_markup=admin_menu())
    except Exception as exc:
        admin_error(m.from_user.id,exc)


def gift_admin_price_save(m):
    try:
        raw=" ".join(str(m.text or "").strip().lower().split())
        if raw in {"free","0","off","disable"}:
            currency="points"; amount=0
        else:
            parts=raw.replace(","," ").split()
            if len(parts)!=2: raise ValueError("Use: points 500, usdt 5, or free")
            currency=parts[0]
            if currency not in {"points","usdt"}: raise ValueError("Currency must be points or usdt")
            amount=float(parts[1])
            if not math.isfinite(amount) or amount<0: raise ValueError("Price must be a valid non-negative number")
            if currency=="points":
                if not amount.is_integer(): raise ValueError("Points price must be a whole number")
                if amount>1000000000: raise ValueError("Points price is too large")
                amount=int(amount)
            else:
                if amount>1000000: raise ValueError("USDT price is too large")
                amount=round(amount,2)
        set_config("gift_price_currency",currency)
        set_config("gift_price_amount",amount)
        raw_bot.send_message(
            m.chat.id,
            f"✅ Gift access price updated to {_gift_price_text()}.\n\nUsers who already purchased remain unlocked; the new price applies to users who have not purchased yet.",
            reply_markup=admin_menu(),
        )
    except Exception as exc:
        admin_error(m.from_user.id,exc)
...
def gift_admin_stats(admin_id):
    limit=_gift_report_limit(); lines=["📊 GIFT LINK STATISTICS","",f"Access price: {_gift_price_text()}"]
    for kind in ("mobile","pc"):
        total=gift_links_col.count_documents({"kind":kind})
        active=gift_links_col.count_documents({"kind":kind,"status":"active","report_count":{"$lt":limit}})
        retired=total-active
        reports=sum(int(r.get("report_count",0) or 0) for r in gift_links_col.find({"kind":kind},{"report_count":1}))
        deliveries=sum(int(r.get("delivery_count",0) or 0) for r in gift_links_col.find({"kind":kind},{"delivery_count":1}))
        lines.append(f"{_gift_kind_label(kind)}\nActive: {active} | Retired: {retired} | Total: {total}\nDeliveries: {deliveries} | Not-working reports: {reports}")
    purchases=gift_access_purchases_col.count_documents({"status":"completed"})
    usdt_revenue=sum(float(r.get("amount",0) or 0) for r in gift_access_purchases_col.find({"status":"completed","currency":"usdt"},{"amount":1}))
    points_revenue=sum(int(r.get("amount",0) or 0) for r in gift_access_purchases_col.find({"status":"completed","currency":"points"},{"amount":1}))
    lines.append(f"\nGift access purchases: {purchases}\nCollected: {points_revenue:,} points and ${usdt_revenue:.2f} USDT")
    lines.append(f"\nAutomatic retirement threshold: {limit} distinct users")
    raw_bot.send_message(admin_id,"\n\n".join(lines),reply_markup=_gift_admin_markup())


def gift_admin_reports(admin_id):
    rows=list(gift_link_reports_col.find({}).sort("created_at",-1).limit(30))
    if not rows: return raw_bot.send_message(admin_id,"🚫 No Not-Working reports yet.",reply_markup=_gift_admin_markup())
    lines=["🚫 RECENT NOT-WORKING REPORTS",""]
    for report in rows:
        link=gift_links_col.find_one({"_id":report.get("link_id")},{"url":1,"report_count":1}) or {}
        url=str(link.get("url") or "Deleted link")
        lines.append(f"• User {report.get('user_id')} • {_gift_kind_label(report.get('kind'))}\nReports on link: {int(link.get('report_count',0) or 0)}\n{url[:180]}")
    raw_bot.send_message(admin_id,"\n\n".join(lines)[:3900],reply_markup=_gift_admin_markup(),disable_web_page_preview=True)


def _gift_not_working_query():
    return {"$or":[{"status":"retired"},{"report_count":{"$gte":_gift_report_limit()}}]}


def gift_admin_links_list(admin_id,kind,page=0):
    if kind not in {"mobile","pc"}:
        return raw_bot.send_message(admin_id,"❌ Invalid Gift link type.",reply_markup=_gift_admin_markup())
    page=max(0,int(page or 0)); page_size=10
    total=gift_links_col.count_documents({"kind":kind})
    if total and page*page_size>=total: page=max(0,(total-1)//page_size)
    rows=list(gift_links_col.find({"kind":kind}).sort([("status",1),("report_count",-1),("created_at",-1)]).skip(page*page_size).limit(page_size))
    title="PHONE" if kind=="mobile" else "PC"
    if not rows:
        return raw_bot.send_message(admin_id,f"📋 {title} LOGIN LINKS\n\nNo links have been imported.",reply_markup=_gift_admin_markup())
    lines=[f"📋 {title} LOGIN LINKS",f"Page {page+1} • Total: {total}",""]
    kb=InlineKeyboardMarkup(row_width=1)
    limit=_gift_report_limit()
    for index,row in enumerate(rows,start=page*page_size+1):
        reports=int(row.get("report_count",0) or 0)
        active=row.get("status","active")=="active" and reports<limit
        state="✅ Active" if active else "🚫 Not working"
        lines.append(f"#{index} • {state}\nReports: {reports} | Deliveries: {int(row.get('delivery_count',0) or 0)}\n{str(row.get('url',''))[:220]}")
        kb.add(InlineKeyboardButton(f"🗑 Delete #{index}",callback_data=f"giftdel|ask|{row['_id']}"))
    nav=[]
    if page>0: nav.append(InlineKeyboardButton("⬅️ Previous",callback_data=f"giftadm|list|{kind}|{page-1}"))
    if (page+1)*page_size<total: nav.append(InlineKeyboardButton("Next ➡️",callback_data=f"giftadm|list|{kind}|{page+1}"))
    if nav: kb.row(*nav)
    kb.add(InlineKeyboardButton("↩️ Gift Manager",callback_data="giftadm|stats"))
    raw_bot.send_message(admin_id,"\n\n".join(lines)[:3900],reply_markup=kb,disable_web_page_preview=True)


def _gift_delete_link_records(link_ids):
    ids=list(link_ids or [])
    if not ids: return 0
    deleted=0
    for start in range(0,len(ids),500):
        batch=ids[start:start+500]
        gift_link_reports_col.delete_many({"link_id":{"$in":batch}})
        gift_link_deliveries_col.delete_many({"link_id":{"$in":batch}})
        deleted+=gift_links_col.delete_many({"_id":{"$in":batch}}).deleted_count
    return deleted


@bot.callback_query_handler(func=lambda c:str(c.data or "").startswith("giftdel|") and is_admin(c.from_user.id))
def gift_admin_delete_link_cb(c):
    try:
        _,action,oid_text=c.data.split("|",2); oid=ObjectId(oid_text)
        row=gift_links_col.find_one({"_id":oid})
        if not row: return bot.answer_callback_query(c.id,"Link already deleted",True)
        kind=row.get("kind","mobile")
        if action=="ask":
            kb=InlineKeyboardMarkup(row_width=2)
            kb.row(
                InlineKeyboardButton("✅ Delete Link",callback_data=f"giftdel|yes|{oid}"),
                InlineKeyboardButton("❌ Cancel",callback_data=f"giftadm|list|{kind}|0"),
            )
            bot.answer_callback_query(c.id)
            return raw_bot.send_message(
                c.from_user.id,
                f"⚠️ DELETE {_gift_kind_label(kind).upper()}?\n\n{row.get('url')}\n\nIts report and delivery records will also be removed. This cannot be undone.",
                reply_markup=kb,
                disable_web_page_preview=True,
            )
        if action!="yes": return bot.answer_callback_query(c.id,"Invalid delete action",True)
        deleted=_gift_delete_link_records([oid])
        log_event("gift_link_deleted",c.from_user.id,oid_text,{"kind":kind,"url":row.get("url")})
        bot.answer_callback_query(c.id,"Link deleted ✅",True)
        raw_bot.send_message(c.from_user.id,f"✅ Deleted {deleted} {_gift_kind_label(kind)} link and its related records.")
        gift_admin_links_list(c.from_user.id,kind,0)
    except Exception as exc:
        bot.answer_callback_query(c.id,("Delete failed: "+str(exc))[:180],True)


def gift_admin_purge_prompt(admin_id):
    query=_gift_not_working_query(); count=gift_links_col.count_documents(query)
    if not count:
        return raw_bot.send_message(admin_id,"✅ There are no retired/not-working links to delete.",reply_markup=_gift_admin_markup())
    phone=gift_links_col.count_documents({"kind":"mobile","$or":query["$or"]})
    pc=gift_links_col.count_documents({"kind":"pc","$or":query["$or"]})
    kb=InlineKeyboardMarkup(row_width=2)
    kb.row(
        InlineKeyboardButton(f"✅ Delete {count} Links",callback_data="giftpurge|yes"),
        InlineKeyboardButton("❌ Cancel",callback_data="giftadm|stats"),
    )
    raw_bot.send_message(
        admin_id,
        f"⚠️ DELETE ALL NOT-WORKING LINKS?\n\nPhone: {phone}\nPC: {pc}\nTotal: {count}\n\nAll related report and delivery records will also be permanently deleted.",
        reply_markup=kb,
    )


@bot.callback_query_handler(func=lambda c:str(c.data or "")=="giftpurge|yes" and is_admin(c.from_user.id))
def gift_admin_purge_confirm_cb(c):
    try:
        ids=[row["_id"] for row in gift_links_col.find(_gift_not_working_query(),{"_id":1})]
        deleted=_gift_delete_link_records(ids)
        log_event("gift_not_working_links_deleted",c.from_user.id,details={"deleted":deleted})
        bot.answer_callback_query(c.id,f"Deleted {deleted} links",True)
        raw_bot.send_message(c.from_user.id,f"✅ Deleted {deleted} retired/not-working Gift links and their related records.",reply_markup=_gift_admin_markup())
    except Exception as exc:
        bot.answer_callback_query(c.id,("Cleanup failed: "+str(exc))[:180],True)


def gift_admin_purge_kind_prompt(admin_id,kind):
    if kind not in {"mobile","pc"}:
        return raw_bot.send_message(admin_id,"❌ Invalid Gift link type.",reply_markup=_gift_admin_markup())
    count=gift_links_col.count_documents({"kind":kind})
    title="PHONE" if kind=="mobile" else "PC"
    if not count:
        return raw_bot.send_message(admin_id,f"✅ There are no {title} links to delete.",reply_markup=_gift_admin_markup())
    reports=gift_link_reports_col.count_documents({"kind":kind})
    deliveries=gift_link_deliveries_col.count_documents({"kind":kind})
    kb=InlineKeyboardMarkup(row_width=2)
    kb.row(
        InlineKeyboardButton(f"✅ Delete All {title}",callback_data=f"giftpurgekind|yes|{kind}"),
        InlineKeyboardButton("❌ Cancel",callback_data=f"giftadm|list|{kind}|0"),
    )
    raw_bot.send_message(
        admin_id,
        f"⚠️ DELETE ALL {title} LOGIN LINKS?\n\nLinks: {count}\nRelated reports: {reports}\nRelated deliveries: {deliveries}\n\nThis permanently deletes every {title} link and cannot be undone.",
        reply_markup=kb,
    )


@bot.callback_query_handler(func=lambda c:str(c.data or "").startswith("giftpurgekind|") and is_admin(c.from_user.id))
def gift_admin_purge_kind_confirm_cb(c):
    try:
        _,action,kind=c.data.split("|",2)
        if action!="yes" or kind not in {"mobile","pc"}:
            return bot.answer_callback_query(c.id,"Invalid cleanup request",True)
        ids=[row["_id"] for row in gift_links_col.find({"kind":kind},{"_id":1})]
        deleted=_gift_delete_link_records(ids)
        title="Phone" if kind=="mobile" else "PC"
        log_event("gift_link_category_deleted",c.from_user.id,details={"kind":kind,"deleted":deleted})
        bot.answer_callback_query(c.id,f"Deleted {deleted} {title} links",True)
        raw_bot.send_message(
            c.from_user.id,
            f"✅ Deleted all {deleted} {title} Gift links and their related report/delivery records.",
            reply_markup=_gift_admin_markup(),
        )
    except Exception as exc:
        bot.answer_callback_query(c.id,("Cleanup failed: "+str(exc))[:180],True)
...
# Keep the Gift entry visible on the main page while allowing its label to be
# changed independently from the generic menu-layout system.
_v36_main_menu_original=main_menu
def main_menu(uid):
    kb=_v36_main_menu_original(uid)
    try: kb.row(_gift_button_name())
    except Exception: pass
    return kb


_v36_admin_menu_original=admin_menu
def admin_menu():
    kb=_v36_admin_menu_original()
    try: kb.row("🎁 Gift Link Manager")
    except Exception: pass
    return kb


BOT_BUILD_VERSION='36.4'


# =========================
# 🧭 FINAL FALLBACK HANDLER
# =========================
# IMPORTANT: Keep this handler LAST. PyTelegramBotAPI uses the first matching
# message handler, so registering the catch-all earlier would swallow newer
# feature handlers such as Order Tracker, Delivery Center, Renewal Center,
# Warranty, Expiry, Rewards, Announcements, Earnings Dashboard and Reviews.
