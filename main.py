...
# Referral earnings / payout ledgers
...
def ensure_indexes():
    index_jobs = [
        ...
        (referral_commissions_col, [("referrer_id", 1), ("created_at", -1)], {}),
        (referral_commissions_col, "subscription_id", {"unique": True, "sparse": True}),
...
def get_config():
    ...
    if not cfg:
        cfg = {
            ...
            "referral_min_withdraw_usdt": 5.0,
            "referral_vip_bonus_target": 10,
            "referral_vip_bonus_usdt": 10.0,
            "vip_buy_message": (
                "💎 GLOBEXOMART VIP MEMBERSHIP\n\n"
                "VIP is built for members who want our premium resources in one place.\n\n"
                "✅ Recorded working methods\n"
                "✅ Files, documents, links & videos\n"
                "✅ Tools & premium accounts\n"
                "✅ Giveaways & member rewards\n"
                "✅ Selling market / opportunities\n"
                "✅ Private guidance from our team\n"
                "✅ Live classes\n"
                "✅ 24/7 support\n"
                "✅ Access to all VIP methods while your membership is active\n\n"
                "Choose the plan that suits you below."
            ),
            "vip_rules_message": (
                "📜 VIP INSTRUCTIONS & RULES\n\n"
                "Welcome to Globexomart VIP. Please use all shared resources responsibly, "
                "follow channel/group rules, and contact support if you need help."
            ),
    ...
    def __init__(self, uid):
        ...
        if not data:
            data = {
                ...
                "referral_commission_earned_usdt": 0.0,
                "referral_bonus_earned_usdt": 0.0,
                "referral_vip_bonus_awarded": False,
                "banned": False,
                "muted": False
            }
...
def start_cmd(m):
    ...
    if is_new_user:
        guide_text = (
            ...
            "🛠 SERVICES — Free services may be free/points-based; paid services are always purchased separately in USDT.\n"
...
@bot.message_handler(func=lambda m: m.text in ["📚 Methods", "🛠 Services"])
@force_join_handler
def open_catalog_menu(m):
    kb = InlineKeyboardMarkup(row_width=2)
    if m.text == "📚 Methods":
        kb.add(
            InlineKeyboardButton("🆓 Free Methods", callback_data="catalog|free"),
            InlineKeyboardButton("💎 VIP Methods", callback_data="catalog|vip"),
        )
        raw_bot.send_message(m.from_user.id, "📚 METHODS\n\nChoose a category:", reply_markup=kb)
    else:
        kb.add(
            InlineKeyboardButton("🆓 Free Service", callback_data="catalog|free_service"),
            InlineKeyboardButton("💵 Paid Service", callback_data="catalog|paid_service"),
        )
        raw_bot.send_message(m.from_user.id, "🛠 SERVICES\n\nChoose a category:", reply_markup=kb)
...
def open_folder(c):
    ...
    if cat == "paid_service" and not owns_item:
        kb = InlineKeyboardMarkup(row_width=1)
        kb.add(InlineKeyboardButton(f"💵 Buy for ${price:g} USDT", callback_data=f"buyitem|{folder_id}"))
        bot.answer_callback_query(c.id, "Paid service", True)
        return raw_bot.send_message(uid, f"💵 {name}\n\nPrice: ${price:g} USDT\nPaid services are purchased separately by all users.", reply_markup=kb)
...
# 🎁 REFERRAL
# =========================
def _referral_unique_vip_buyers(uid):
    try:
        return len(referral_commissions_col.distinct("buyer_id", {"referrer_id": int(uid)}))
    except Exception:
        return 0
...
def send_referral_card(uid):
    ...
    commission_pct = min(15.0, max(0.0, float(cfg.get("referral_commission_percent", 15) or 0)))
    vip_buyers = _referral_unique_vip_buyers(uid)
    bonus_target = max(1, int(cfg.get("referral_vip_bonus_target", 10) or 10))
    bonus_amount = float(cfg.get("referral_vip_bonus_usdt", 10) or 10)
    minimum = float(cfg.get("referral_min_withdraw_usdt", 5) or 5)
    available = float(user.data.get("referral_balance_usdt", 0) or 0)
    commission_earned = float(user.data.get("referral_commission_earned_usdt", 0) or 0)
    bonus_earned = float(user.data.get("referral_bonus_earned_usdt", 0) or 0)
    total_earned = float(user.data.get("referral_total_earned_usdt", 0) or 0)
    progress = min(100, int((vip_buyers / bonus_target) * 100))
    filled = min(10, progress // 10)
    bar = "🟩" * filled + "⬜" * (10 - filled)

    text = (
        "💸  GLOBEXOMART REFER & EARN  💸\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Turn your network into real earnings. Share your personal link with friends, communities and customers. "
        "When someone joins through your link and purchases VIP, you earn a commission automatically.\n\n"
        f"🔥 Earn up to *{commission_pct:g}%* from every paid VIP plan your referrals buy.\n"
        f"🏆 Get an extra *${bonus_amount:g} prize* when *{bonus_target} unique referred users* become paid VIP members.\n"
        "♻️ If a referred VIP member renews later, the renewal can earn commission again.\n\n"
        f"🔗 *Your personal referral link:*\n{link}\n\n"
        "📊  YOUR EARNINGS\n"
        f"{bar}  {vip_buyers}/{bonus_target} paid VIP referrals\n"
        f"👥 Verified referrals: {refs}\n"
        f"💵 VIP commission earned: ${commission_earned:.2f}\n"
        f"🏆 Bonus earned: ${bonus_earned:.2f}\n"
        f"📈 Total referral earnings: ${total_earned:.2f}\n"
        f"💰 Available to withdraw: *${available:.2f}*\n"
        f"📤 Minimum withdrawal: ${minimum:.2f}\n\n"
        "The more genuine buyers you refer, the more you can earn. Share your link and build recurring referral income. 🚀"
    )
    share_text = "Join Globexomart VIP and unlock premium resources"
    share_url = f"https://t.me/share/url?url={link}&text={share_text.replace(' ', '%20')}"
    kb = InlineKeyboardMarkup(row_width=2)
    kb.row(
        InlineKeyboardButton("📤 Share Link", url=share_url),
        InlineKeyboardButton("💸 Withdraw Earnings", callback_data="refwithdraw"),
    )
    kb.row(InlineKeyboardButton("🔄 Refresh", callback_data="open_referral_card"))
    raw_bot.send_message(uid, text, reply_markup=kb, disable_web_page_preview=True, parse_mode="Markdown")
...
@bot.callback_query_handler(func=lambda c: c.data == "refwithdraw")
def referral_withdraw_start(c):
    try:
        uid = int(c.from_user.id)
        row = users_col.find_one({"_id": str(uid)}) or {}
        balance = float(row.get("referral_balance_usdt", 0) or 0)
        minimum = float(get_cached_config().get("referral_min_withdraw_usdt", 5) or 5)
        if balance < minimum:
            return bot.answer_callback_query(c.id, f"Minimum withdrawal is ${minimum:.2f}", True)
        _ref_withdraw_state[uid] = {"balance": balance}
        msg = raw_bot.send_message(uid, f"💸 REFERRAL WITHDRAWAL\n\nAvailable: ${balance:.2f}\nMinimum: ${minimum:.2f}\n\nSend the amount you want to withdraw.")
        bot.register_next_step_handler(msg, referral_withdraw_amount_step)
        bot.answer_callback_query(c.id)
    except Exception as exc:
        bot.answer_callback_query(c.id, "Could not start withdrawal", True)
        log_event("referral_withdraw_start_error", c.from_user.id, details={"error": str(exc)}, level="error")

def referral_withdraw_amount_step(m):
    ...
    try:
        ...
        balance = float(row.get("referral_balance_usdt", 0) or 0)
        if amount < minimum:
            raise ValueError(f"Minimum withdrawal is ${minimum:.2f}")
        if amount > balance:
            raise ValueError("Amount is higher than your referral balance")
        _ref_withdraw_state[uid] = {"amount": amount}
        msg = raw_bot.send_message(uid, "Send your USDT wallet address / payout details:")
        bot.register_next_step_handler(msg, referral_withdraw_address_step)
    except Exception as exc:
        _ref_withdraw_state.pop(uid, None)
        raw_bot.send_message(uid, f"❌ {exc}")

def referral_withdraw_address_step(m):
    uid = int(m.from_user.id)
    try:
        st = _ref_withdraw_state.pop(uid, None) or {}
        amount = float(st.get("amount", 0))
        address = (m.text or "").strip()
        if amount <= 0 or len(address) < 4:
        ...
        updated = users_col.find_one_and_update(
            {"_id": str(uid), "referral_balance_usdt": {"$gte": amount}},
            {"$inc": {"referral_balance_usdt": -amount}},
            return_document=ReturnDocument.AFTER,
        )
        if not updated:
            raise ValueError("Referral balance changed or is insufficient")
        doc = {
            "user_id": uid, "username": m.from_user.username, "amount": amount,
            "address": address, "status": "pending", "created_at": time.time(),
        }
        rid = referral_withdrawals_col.insert_one(doc).inserted_id
        kb = InlineKeyboardMarkup(row_width=2)
        kb.add(InlineKeyboardButton("✅ Paid / Approve", callback_data=f"refwdapprove|{rid}"),
               InlineKeyboardButton("❌ Reject", callback_data=f"refwdreject|{rid}"))
        display = f"@{m.from_user.username}" if m.from_user.username else str(uid)
        for admin in get_all_admins():
            try:
                raw_bot.send_message(int(admin["_id"]), f"💸 REFERRAL WITHDRAWAL\n\nUser: {display}\nID: {uid}\nAmount: ${amount:.2f}\nPayout details: {address}", reply_markup=kb)
            except Exception:
                pass
        raw_bot.send_message(uid, f"✅ Withdrawal request submitted for ${amount:.2f}. Admin will review it.")
    except Exception as exc:
        raw_bot.send_message(uid, f"❌ {exc}")

@bot.callback_query_handler(func=lambda c: c.data.startswith(("refwdapprove|", "refwdreject|")))
def referral_withdraw_review(c):
    if not is_admin(c.from_user.id):
        return bot.answer_callback_query(c.id, "Admins only", True)
    try:
        from bson import ObjectId
        action, rid = c.data.split("|", 1)
        row = referral_withdrawals_col.find_one({"_id": ObjectId(rid)})
        if not row or row.get("status") != "pending":
            return bot.answer_callback_query(c.id, "Already reviewed or not found", True)
        approved = action == "refwdapprove"
        status = "paid" if approved else "rejected"
        referral_withdrawals_col.update_one({"_id": row["_id"], "status": "pending"}, {"$set": {"status": status, "reviewed_at": time.time(), "reviewed_by": c.from_user.id}})
        if not approved:
            users_col.update_one({"_id": str(row["user_id"])}, {"$inc": {"referral_balance_usdt": float(row["amount"])}})
        bot.answer_callback_query(c.id, "Approved" if approved else "Rejected")
        raw_bot.edit_message_text(c.message.text + ("\n\n✅ PAID" if approved else "\n\n❌ REJECTED / BALANCE RETURNED"), c.message.chat.id, c.message.message_id)
        try:
            raw_bot.send_message(int(row["user_id"]), f"{'✅' if approved else '❌'} Referral withdrawal ${float(row['amount']):.2f} was {'paid/approved' if approved else 'rejected. The amount was returned to your referral balance'}.")
        except Exception:
            pass
    except Exception as exc:
        admin_error(c.from_user.id, exc)
...
def buy_vip_button(m):
    ...
    if current:
        exp = "Lifetime" if current.get("expires_at") is None else datetime.fromtimestamp(current["expires_at"]).strftime("%Y-%m-%d %H:%M")
        return raw_bot.send_message(uid, f"✅ GLOBEXOMART ACCESS ACTIVE\n\nPlan: {current.get('plan')}\nExpires: {exp}")

    plans = get_subscription_plans() if "get_subscription_plans" in globals() else {
        "1M": {"days": 30, "price": 25}, "2M": {"days": 60, "price": 40},
        "4M": {"days": 120, "price": 60}, "1Y": {"days": 365, "price": 100}
    }
    kb = InlineKeyboardMarkup(row_width=2)
    labels = {"1M": "1 Month • $25", "2M": "2 Months • $40", "4M": "4 Months • $60", "1Y": "1 Year • $100"}
    for code in ("1M", "2M", "4M", "1Y"):
        if code in plans:
            kb.add(InlineKeyboardButton(labels.get(code, code), callback_data=f"subplan|{code}"))
    cfg = get_cached_config()
    buy_message = cfg.get("vip_buy_message") or (
        "💎 GLOBEXOMART VIP MEMBERSHIP\n\n"
        "✅ Recorded working methods\n✅ Files, documents, links & videos\n✅ Tools & premium accounts\n"
        "✅ Giveaways & selling market\n✅ Private guidance\n✅ Live classes\n✅ 24/7 support\n\n"
        "Choose your plan below."
    )
    dynamic_plans = []
    for code in ("1M", "2M", "4M", "1Y"):
        if code in plans:
            dynamic_plans.append(f"• {code}: ${float(plans[code]['price']):g} — {int(plans[code]['days'])} days")
    raw_bot.send_message(uid, buy_message + "\n\n💰 VIP PLANS\n" + "\n".join(dynamic_plans), reply_markup=kb)
...
def admin_menu():
    ...
    kb.row("🆓 Upload Free Service", "💵 Upload Paid Service")
...
def upload_text_name(m, cat, is_service):
    name = m.text
    msg = bot.send_message(m.from_user.id, "💰 **Price**\nFree/Free Service: points (0 = free)\nVIP/Paid Service: USDT:", parse_mode="Markdown")
...
@bot.message_handler(func=lambda m: m.text in ["📦 Upload Free Method", "💎 Upload VIP Method", "🆓 Upload Free Service", "💵 Upload Paid Service"] and is_admin(m.from_user.id))
def upload_handler(m):
    cats = {
        "📦 Upload Free Method": ("free", False),
        "💎 Upload VIP Method": ("vip", False),
        "🆓 Upload Free Service": ("free_service", True),
        "💵 Upload Paid Service": ("paid_service", True),
    }
...
def _parse_custom_vip_duration(value):
    raw = str(value or "").strip().lower().replace(" ", "")
    if raw in ("lifetime", "life", "permanent", "forever", "0"):
        return None, "Lifetime"
    match = re.fullmatch(r"(\d+(?:\.\d+)?)(m|min|mins|minute|minutes|h|hr|hrs|hour|hours|d|day|days|w|week|weeks|mo|month|months|y|yr|year|years)", raw)
    if not match:
        raise ValueError("Use examples: 30m, 2h, 7d, 2w, 1mo, 1y, lifetime")
...
def add_vip_process(m):
    ...
    if inp.startswith("@"):
        try:
            ...
            return raw_bot.send_message(m.from_user.id, "❌ User not found!")
    else:
        try:
            target = int(inp)
        except Exception:
            return raw_bot.send_message(m.from_user.id, "❌ Invalid ID!")
    msg = raw_bot.send_message(m.from_user.id,
        "⏱ Send VIP duration.\n\nExamples:\n30m = 30 minutes\n2h = 2 hours\n7d = 7 days\n2w = 2 weeks\n1mo = 1 month\n1y = 1 year\nlifetime = never expires")
    bot.register_next_step_handler(msg, lambda x: add_vip_duration_process(x, target))

def add_vip_duration_process(m, target):
    try:
        seconds, label = _parse_custom_vip_duration(m.text)
        now = time.time()
        current = _active_subscription(target) if "_active_subscription" in globals() else None
        base = now
        if current and current.get("expires_at"):
            base = max(now, float(current["expires_at"]))
        expires_at = None if seconds is None else base + seconds
        subscriptions_col.update_many({"user_id": int(target), "status": "active"}, {"$set": {"status": "superseded", "superseded_at": now}})
        sub_doc = {
            "user_id": int(target), "plan": "CUSTOM", "duration_label": str(label),
            "days": None if seconds is None else seconds / 86400, "price_usd": 0.0,
            "payment_mode": "admin_grant", "status": "active", "starts_at": now,
            "expires_at": expires_at, "created_at": now, "added_by": m.from_user.id,
        }
        subscriptions_col.insert_one(sub_doc)
        _sync_user_vip(target, expires_at)
        links = _grant_chat_access(target)
        exp_text = "Lifetime" if expires_at is None else datetime.fromtimestamp(expires_at).strftime("%Y-%m-%d %H:%M")
        raw_bot.send_message(m.from_user.id, f"✅ User {target} is now VIP\nDuration: {label}\nExpires: {exp_text}", reply_markup=admin_menu())
        _send_vip_instructions(target)
        good_links = [link for _, link, err in links if link and not err]
        user_text = f"🎉 GLOBEXOMART VIP ACTIVE\n\nDuration: {label}\nExpires: {exp_text}"
        if good_links:
            user_text += "\n\nYour private one-time join link(s):\n" + "\n".join(good_links)
        try:
            raw_bot.send_message(target, user_text, disable_web_page_preview=True)
        except Exception:
            pass
    except Exception as exc:
...
def vip_settings_menu(m):
    ...
    for code in ("1M", "2M", "4M", "1Y"):
        kb.add(
            InlineKeyboardButton(f"💰 {code} Price", callback_data=f"vipplanprice|{code}"),
            InlineKeyboardButton(f"📅 {code} Days", callback_data=f"vipplandays|{code}"),
        )
    kb.add(
    ...
    kb.add(
        InlineKeyboardButton("📝 VIP Buy Message", callback_data="set_vip_buy_message"),
        InlineKeyboardButton("📜 VIP Rules", callback_data="set_vip_rules_message"),
    )
    bot.send_message(m.from_user.id, "⚙️ **VIP Settings**\n\nEdit each plan price and interval:", reply_markup=kb, parse_mode="Markdown")


@bot.callback_query_handler(func=lambda c: c.data == "set_vip_buy_message")
def set_vip_buy_message_cb(c):
    if not is_admin(c.from_user.id): return
    msg = raw_bot.send_message(c.from_user.id, "Send the complete VIP explanation users should see before choosing a plan:")
    bot.register_next_step_handler(msg, lambda m: _save_long_config_text(m, "vip_buy_message", "VIP buy message"))
    bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data == "set_vip_rules_message")
def set_vip_rules_message_cb(c):
    if not is_admin(c.from_user.id): return
    msg = raw_bot.send_message(c.from_user.id, "Send the VIP Instructions & Rules message. It will be sent after VIP activation:")
    bot.register_next_step_handler(msg, lambda m: _save_long_config_text(m, "vip_rules_message", "VIP rules"))
    bot.answer_callback_query(c.id)

def _save_long_config_text(m, key, label):
    text = (m.text or m.caption or "").strip()
    if not text:
        return raw_bot.send_message(m.from_user.id, "❌ Message cannot be empty.", reply_markup=admin_menu())
    set_config(key, text[:12000])
    raw_bot.send_message(m.from_user.id, f"✅ {label} updated.", reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "🎁 Referral Settings" and is_admin(m.from_user.id))
def referral_settings_menu(m):
    cfg = get_cached_config()
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(InlineKeyboardButton("📈 Commission %", callback_data="refset|commission"),
           InlineKeyboardButton("💸 Min Withdraw", callback_data="refset|minwithdraw"))
    kb.add(InlineKeyboardButton("🏆 Prize Amount", callback_data="refset|bonus"),
           InlineKeyboardButton("👥 Prize Target", callback_data="refset|target"))
    text = (f"🎁 REFERRAL SETTINGS\n\nCommission: {float(cfg.get('referral_commission_percent',15)):g}% (max 15%)\n"
            f"Minimum withdrawal: ${float(cfg.get('referral_min_withdraw_usdt',5)):.2f}\n"
            f"VIP buyer prize: ${float(cfg.get('referral_vip_bonus_usdt',10)):.2f} after {int(cfg.get('referral_vip_bonus_target',10))} unique paid VIP buyers")
    raw_bot.send_message(m.from_user.id, text, reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data.startswith("refset|"))
def referral_setting_cb(c):
    if not is_admin(c.from_user.id): return bot.answer_callback_query(c.id, "Admin only", True)
    action = c.data.split("|",1)[1]
    prompts = {
        "commission":"Send commission percentage from 0 to 15:",
        "minwithdraw":"Send minimum referral withdrawal in USDT:",
        "bonus":"Send prize amount in USDT:",
        "target":"Send number of unique paid VIP referrals required for the prize:",
    }
    msg = raw_bot.send_message(c.from_user.id, prompts[action])
    bot.register_next_step_handler(msg, lambda m: referral_setting_save(m, action))
    bot.answer_callback_query(c.id)
...
def referral_setting_save(m, action):
    try:
        if action == "target":
            ...
            if value < 1: raise ValueError("Target must be at least 1")
            set_config("referral_vip_bonus_target", value)
        else:
            value = float((m.text or "").strip())
            if value < 0: raise ValueError("Value cannot be negative")
            if action == "commission":
                if value > 15: raise ValueError("Commission cannot be more than 15%")
                set_config("referral_commission_percent", value)
            elif action == "minwithdraw": set_config("referral_min_withdraw_usdt", value)
            elif action == "bonus": set_config("referral_vip_bonus_usdt", value)
        raw_bot.send_message(m.from_user.id, "✅ Referral setting updated.", reply_markup=admin_menu())
    except Exception as exc:
        raw_bot.send_message(m.from_user.id, f"❌ {exc}", reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "💸 Ref Withdrawals" and is_admin(m.from_user.id))
def referral_withdrawals_admin(m):
    rows = list(referral_withdrawals_col.find({}).sort("created_at", -1).limit(20))
    if not rows:
        return raw_bot.send_message(m.from_user.id, "💸 No referral withdrawals yet.")
    text = "💸 RECENT REFERRAL WITHDRAWALS\n\n"
    for row in rows:
        text += f"• {row.get('user_id')} — ${float(row.get('amount',0)):.2f} — {row.get('status')}\n"
    raw_bot.send_message(m.from_user.id, text)
...
def view_vip_plans_cb(c):
    ...
    for code in ("1M","2M","4M","1Y"):
        row = plans.get(code)
        if row:
            lines.append(f"{code}: ${row['price']:g} USDT • {row['days']} days")
...
def get_subscription_plans():
    ...
    for code, row in plans.items():
        try:
            normalized[str(code).upper()] = {"days": int(row["days"]), "price": float(row["price"])}
        except Exception:
            continue
    return normalized or SUBSCRIPTION_PLANS.copy()
...
def _send_vip_instructions(uid):
    text = (get_cached_config().get("vip_rules_message") or "").strip()
    if not text:
        return
    try:
        raw_bot.send_message(int(uid), text, disable_web_page_preview=True)
    except Exception as exc:
        log_event("vip_rules_send_error", uid, details={"error": str(exc)}, level="error")

def _credit_referral_vip_sale(buyer_id, subscription_id, amount, plan_code):
    """Credit commission once per paid subscription and award the unique-buyer milestone prize once."""
    try:
        buyer_id = int(buyer_id)
        buyer = users_col.find_one({"_id": str(buyer_id)}) or {}
        ref_id = buyer.get("ref")
        if not ref_id or not str(ref_id).isdigit() or int(ref_id) == buyer_id:
            return None
        referrer_id = int(ref_id)
        cfg = get_cached_config()
        pct = min(15.0, max(0.0, float(cfg.get("referral_commission_percent", 15) or 0)))
        commission = round(float(amount or 0) * pct / 100.0, 4)
        if commission <= 0:
            return None
        doc = {
            "subscription_id": str(subscription_id), "referrer_id": referrer_id, "buyer_id": buyer_id,
            "plan": str(plan_code), "sale_amount": float(amount or 0), "percent": pct,
            "commission": commission, "created_at": time.time(),
        }
        try:
            referral_commissions_col.insert_one(doc)
        except Exception:
            # Unique subscription id prevents accidental double-credit.
            if referral_commissions_col.find_one({"subscription_id": str(subscription_id)}):
                return None
            raise
        users_col.update_one({"_id": str(referrer_id)}, {"$inc": {
            "referral_balance_usdt": commission,
            "referral_total_earned_usdt": commission,
            "referral_commission_earned_usdt": commission,
        }})
        unique_buyers = len(referral_commissions_col.distinct("buyer_id", {"referrer_id": referrer_id}))
        users_col.update_one({"_id": str(referrer_id)}, {"$set": {"refs_who_bought_vip": unique_buyers}})
        target = max(1, int(cfg.get("referral_vip_bonus_target", 10) or 10))
        bonus = float(cfg.get("referral_vip_bonus_usdt", 10) or 10)
        bonus_awarded = False
        if unique_buyers >= target and bonus > 0:
            claimed = users_col.update_one(
                {"_id": str(referrer_id), "referral_vip_bonus_awarded": {"$ne": True}},
                {"$set": {"referral_vip_bonus_awarded": True, "referral_vip_bonus_awarded_at": time.time()},
                 "$inc": {"referral_balance_usdt": bonus, "referral_total_earned_usdt": bonus, "referral_bonus_earned_usdt": bonus}},
            )
            bonus_awarded = claimed.modified_count == 1
        User._cache.pop(str(referrer_id), None); User._cache_time.pop(str(referrer_id), None)
        try:
            note = (f"💸 REFERRAL COMMISSION EARNED!\n\nA user you referred purchased VIP.\nPlan: {plan_code}\n"
                    f"Sale: ${float(amount):.2f}\nYour commission ({pct:g}%): +${commission:.2f}\n"
                    f"Unique paid VIP referrals: {unique_buyers}/{target}")
            if bonus_awarded:
                note += f"\n\n🏆 MILESTONE PRIZE: +${bonus:.2f} for reaching {target} unique paid VIP referrals!"
            raw_bot.send_message(referrer_id, note)
        except Exception:
            pass
        return doc
    except Exception as exc:
        log_event("referral_vip_commission_error", buyer_id, details={"error": str(exc)}, level="error")
        return None
...
def activate_subscription(uid, plan_code, payment_mode="manual", amount=None, payment_ref=None, added_by=None):
    ...
    # Referral commission applies only to real paid/manual-auto subscriptions with a payment reference.
    if charged > 0 and payment_ref and str(payment_mode).lower() in ("manual", "auto"):
        _credit_referral_vip_sale(uid, sub_id, charged, code)
    _send_vip_instructions(uid)
...
def _subscription_status_text(uid):
    ...
    exp = "Lifetime" if sub.get("expires_at") is None else datetime.fromtimestamp(sub["expires_at"]).strftime("%Y-%m-%d %H:%M")
...
def choose_subscription_plan_cb(c):
    try:
        ...
        cfg = get_cached_config()
        kb = InlineKeyboardMarkup(row_width=1)
        kb.add(InlineKeyboardButton("🧾 Request Manual Payment", callback_data=f"submanual|{code}"))
...
def request_manual_subscription_payment_cb(c):
    try:
        ...
        if pending:
            return bot.answer_callback_query(c.id, "Payment request already pending", True)
        doc = {
        ...
        kb = InlineKeyboardMarkup(row_width=2)
        kb.add(
            InlineKeyboardButton("✅ Approve", callback_data=f"payapprove|{pid}"),
            InlineKeyboardButton("❌ Reject", callback_data=f"payreject|{pid}"),
        )
        display = f"@{c.from_user.username}" if c.from_user.username else str(c.from_user.id)
        for admin in get_all_admins():
            try:
                raw_bot.send_message(
                    int(admin["_id"]),
                    f"🧾 MANUAL PAYMENT REQUEST\n\nUser: {display}\nID: {c.from_user.id}\nPlan: {code}\nAmount: ${plan['price']}\n\nVerify the payment independently before approving.",
                    reply_markup=kb,
                )
            except Exception:
                pass
        raw_bot.send_message(c.from_user.id, "🧾 Payment request sent to the admin. Send your payment proof to the admin and wait for approval.")
        bot.answer_callback_query(c.id, "Request sent")
    except Exception as exc:
        bot.answer_callback_query(c.id, "Request failed", True)
...
@bot.message_handler(commands=["access_add"])
...
def buy_item_usdt_cb(c):
    ...
    try:
        ...
        if folder.get("cat") == "vip" and User(c.from_user.id).is_vip():
        ...
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("📸 Submit Payment Screenshot", callback_data="itempay_screenshot"))
        raw_bot.send_message(c.from_user.id, f"💵 BUY {folder.get('name')}\n\n{_usdt_instructions(price)}\n\nAfter payment, submit your screenshot.", reply_markup=kb)
        bot.answer_callback_query(c.id)
    except Exception as exc:
...
def item_payment_receive_screenshot(m):
    ...
    doc = {
        "user_id": int(m.from_user.id), "folder_id": state["folder_id"], "item_name": folder.get("name"),
        "category": folder.get("cat"), "amount": float(state["price"]), "currency": "USDT",
        "status": "pending", "screenshot_chat_id": m.chat.id, "screenshot_message_id": m.message_id,
        "created_at": time.time()
    }
    ...
    for admin in get_all_admins():
        try:
            aid = int(admin["_id"])
            raw_bot.send_message(aid, f"💵 ITEM PAYMENT\n\nUser: {m.from_user.id}\nItem: {folder.get('name')}\nCategory: {folder.get('cat')}\nAmount: ${state['price']:g} USDT", reply_markup=kb)
            raw_bot.copy_message(aid, m.chat.id, m.message_id)
        except Exception:
            pass
    raw_bot.send_message(m.chat.id, "✅ Payment submitted. You will be notified after admin review.")
...
def review_item_payment(c):
    ...
    if approved:
        payments_col.insert_one({"user_id": row["user_id"], "type": "item", "folder_id": row["folder_id"], "amount": row["amount"], "currency": "USDT", "mode": "manual", "status": "paid", "created_at": time.time(), "approved_by": c.from_user.id})
    try:
        raw_bot.send_message(row["user_id"], ("✅ Payment approved. You can now open " + row["item_name"] + ".") if approved else "❌ Payment rejected. Please contact admin if needed.")
    except Exception:
        pass
...
def deposit_amount_step(m):
    try:
        ...
        msg = raw_bot.send_message(m.chat.id, "Now send the payment screenshot.")
...
def deposit_screenshot_step(m):
    ...
    if not st or m.content_type != "photo":
        return raw_bot.send_message(m.chat.id, "Deposit cancelled. A screenshot photo is required.")
    doc = {"user_id": int(m.from_user.id), "type": "deposit", "amount": st["amount"], "status": "pending", "created_at": time.time(), "screenshot_chat_id": m.chat.id, "screenshot_message_id": m.message_id}
    ...
    for admin in get_all_admins():
        try:
            aid = int(admin["_id"])
            raw_bot.send_message(aid, f"💳 DEPOSIT REQUEST\nUser: {m.from_user.id}\nAmount: ${st['amount']:.2f} USDT", reply_markup=kb)
            raw_bot.copy_message(aid, m.chat.id, m.message_id)
        except Exception:
            pass
    raw_bot.send_message(m.chat.id, "✅ Deposit submitted for approval.")
...
def discount_admin(m):
    cfg = get_config()
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(InlineKeyboardButton("🔄 Toggle", callback_data="discount_toggle"),
           InlineKeyboardButton("✏️ Set %", callback_data="discount_set"))
    raw_bot.send_message(m.chat.id, f"🏷 DISCOUNT\n\nStatus: {'ON' if cfg.get('discount_enabled') else 'OFF'}\nPercent: {cfg.get('discount_percent',0)}%", reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data == "discount_toggle")
def discount_toggle(c):
    if not is_admin(c.from_user.id):
        return
    set_config("discount_enabled", not bool(get_config().get("discount_enabled")))
    bot.answer_callback_query(c.id, "Updated", True)

@bot.callback_query_handler(func=lambda c: c.data == "discount_set")
def discount_set(c):
    if not is_admin(c.from_user.id):
        return
    msg = raw_bot.send_message(c.from_user.id, "Send discount percent (0-100):")
    bot.register_next_step_handler(msg, discount_set_step)
    bot.answer_callback_query(c.id)

def discount_set_step(m):
    try:
        pct = float(m.text)
        if pct < 0 or pct > 100:
            raise ValueError()
        set_config("discount_percent", pct)
        raw_bot.send_message(m.chat.id, f"✅ Discount set to {pct:g}%.", reply_markup=admin_menu())
    except Exception:
...
def _send_daily_owner_report():
    ...
    now = time.time()
    day_ago = now - 86400
    active = subscriptions_col.count_documents({"status": "active", "expires_at": {"$gt": now}})
    expiring = subscriptions_col.count_documents({"status": "active", "expires_at": {"$gt": now, "$lte": now + 86400}})
    new_subs = subscriptions_col.count_documents({"created_at": {"$gte": day_ago}})
    pay_docs = list(payments_col.find({"created_at": {"$gte": day_ago}, "status": "paid"}))
    revenue = sum(float(x.get("amount", 0) or 0) for x in pay_docs)
    manual = sum(1 for x in pay_docs if x.get("mode") == "manual")
    auto = sum(1 for x in pay_docs if x.get("mode") == "auto")
    users = users_col.count_documents({})
    text = (
        "📊 GLOBEXOMART DAILY REPORT\n\n"
        f"👥 Total bot users: {users}\n"
        f"💎 Active subscriptions: {active}\n"
        f"🆕 New subscriptions (24h): {new_subs}\n"
        f"⏳ Expiring in next 24h: {expiring}\n"
        f"💰 Payments (24h): {len(pay_docs)}\n"
        f"💵 Revenue recorded: ${revenue:.2f}\n"
        f"🧾 Manual payments: {manual}\n"
        f"⚡ Auto payments: {auto}"
    )
    raw_bot.send_message(ADMIN_ID, text)
