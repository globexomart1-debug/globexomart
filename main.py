...
def _clear_known_flow_state(uid, chat_id=None):
    ...
    transient_names = (
        ...
        "_wallet_state", "_ref_withdraw_state", "_auto_broadcast_state",
    )
...
def buy_vip_button(m):
    ...
    plan_lines = []
    label_names = {"1M":"1 Month","2M":"2 Months","4M":"4 Months","1Y":"1 Year"}
    for code in ("1M","2M","4M","1Y"):
        if code in plans:
            plan_lines.append(f"• {label_names.get(code, code)} — ${float(plans[code].get('price',0)):g}")
    raw_bot.send_message(
        uid,
        f"{intro}\n\n💳 VIP PLANS\n" + "\n".join(plan_lines) +
        "\n\n✅ After admin approves your payment, VIP is activated automatically and a private one-user invite link is sent to you.\n"
        "⏳ Timed VIP access is removed automatically when the interval ends.",
        reply_markup=kb,
    )
