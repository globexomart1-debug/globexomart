...
def _main_order_markup(page=0, selected=None):
    ...
    if nav:
        kb.row(*nav)
    current_cols = get_main_menu_columns()
    kb.row(
        InlineKeyboardButton(("✅ " if current_cols == 1 else "") + "1 / row", callback_data="mmord|cols|1"),
        InlineKeyboardButton(("✅ " if current_cols == 2 else "") + "2 / row", callback_data="mmord|cols|2"),
        InlineKeyboardButton(("✅ " if current_cols == 3 else "") + "3 / row", callback_data="mmord|cols|3")
    )
    kb.row(
...
def main_button_order_callback(c):
    ...
    try:
        ...
        if action == "cols":
            ...
            kb, _, _ = _main_order_markup(0, None)
            raw_bot.edit_message_text(_main_order_text(), c.message.chat.id, c.message.message_id, reply_markup=kb)
            return bot.answer_callback_query(c.id, f"Layout set to {n} per row", True)
...
def v28_more_hub(m):
    ...
    raw_bot.send_message(m.chat.id,'🧰 MORE TOOLS\n\nLearning progress, VIP comparison, saved notes, alerts and affiliate information in one place.',reply_markup=kb)
