...
def _scanner_visibility_allowed(row, uid):
    vis = str(row.get("visibility") or "all").lower()
    if vis == "hidden" or not row.get("active", True):
        return False
    if is_admin(uid):
        return True
    if vis == "vip":
...
def scanner_admin_menu(m):
    rows = _scanner_rows()
    raw_bot.send_message(m.from_user.id, f"🔎 SCANNER MANAGER\n━━━━━━━━━━━━━━━━━━━━\nListings: {len(rows)}\n\nAdd scanner profiles with platform, username/chat, price and visibility.", reply_markup=_scanner_admin_keyboard())
...
def scanner_add_platform_step(m):
    ...
    msg=raw_bot.send_message(m.chat.id,"Step 2/5: Send scanner username.\nExample: @scannername")
...
def scanner_add_username_step(m):
    ...
    msg=raw_bot.send_message(m.chat.id,"Step 3/5: Send Telegram public username/link users should open to chat.\nExample: @scannername or https://t.me/scannername")
...
def scanner_add_link_step(m):
    ...
    msg=raw_bot.send_message(m.chat.id,"Step 4/5: Send scan price in USD/USDT.\nExample: 5 or 2.5")
...
def scanner_add_price_step(m):
    ...
    state["price"] = price
    kb=InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("🌍 Visible to All",callback_data="scanvisnew|all"))
    kb.add(InlineKeyboardButton("👑 VIP Users Only",callback_data="scanvisnew|vip"))
    kb.add(InlineKeyboardButton("🙈 Hidden",callback_data="scanvisnew|hidden"))
    raw_bot.send_message(m.chat.id,"Step 5/5: Who can see this scanner?",reply_markup=kb)

@bot.callback_query_handler(func=lambda c: c.data.startswith("scanvisnew|"))
def scanner_add_visibility_cb(c):
    if not is_admin(c.from_user.id): return bot.answer_callback_query(c.id,"Admins only",True)
    state=_scanner_admin_state.pop(c.from_user.id,None)
    if not state: return bot.answer_callback_query(c.id,"Session expired",True)
    vis=c.data.split("|",1)[1]
    row={
        ...
        "price": float(state["price"]), "visibility": vis, "active": True,
        "created_at": time.time(), "created_by": int(c.from_user.id)
    }
    rows=_scanner_rows(); rows.append(row); _save_scanner_rows(rows)
    bot.answer_callback_query(c.id,"Scanner added",True)
    raw_bot.send_message(c.from_user.id,f"✅ SCANNER ADDED\n\nPlatform: {row['platform']}\nScanner: {row['username']}\nPrice: ${row['price']:g}\nVisibility: {vis.upper()}",reply_markup=admin_menu())
...
def scanner_manage_cb(c):
    ...
    for i,row in enumerate(rows):
        vis={"all":"🌍","vip":"👑","hidden":"🙈"}.get(str(row.get("visibility")),"🌍")
        kb.add(InlineKeyboardButton(f"{vis} {row.get('platform')} • {row.get('username')} • ${float(row.get('price',0) or 0):g}",callback_data=f"scanedit|{i}"))
...
def scanner_edit_cb(c):
    ...
    kb=InlineKeyboardMarkup(row_width=2)
    kb.row(InlineKeyboardButton("🌍 All",callback_data=f"scanvis|{idx}|all"), InlineKeyboardButton("👑 VIP",callback_data=f"scanvis|{idx}|vip"))
    kb.row(InlineKeyboardButton("🙈 Hide",callback_data=f"scanvis|{idx}|hidden"), InlineKeyboardButton("🗑 Delete",callback_data=f"scandel|{idx}"))
    kb.add(InlineKeyboardButton("⬅️ Back",callback_data="scanadm|manage"))
    raw_bot.edit_message_text(f"🔎 SCANNER\n\nPlatform: {row.get('platform')}\nUsername: {row.get('username')}\nPrice: ${float(row.get('price',0) or 0):g}\nVisibility: {str(row.get('visibility','all')).upper()}\nChat: {row.get('chat_link')}",c.from_user.id,c.message.message_id,reply_markup=kb)
    bot.answer_callback_query(c.id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("scanvis|"))
def scanner_change_visibility_cb(c):
    if not is_admin(c.from_user.id): return bot.answer_callback_query(c.id,"Admins only",True)
    try:
        _,idxs,vis=c.data.split("|",2); idx=int(idxs); rows=_scanner_rows(); rows[idx]["visibility"]=vis; _save_scanner_rows(rows)
        bot.answer_callback_query(c.id,f"Visibility: {vis.upper()}",True)
        c.data=f"scanedit|{idx}"; return scanner_edit_cb(c)
    except Exception as exc:
        bot.answer_callback_query(c.id,"Unable to update",True); log_event("scanner_visibility_error",c.from_user.id,details={"error":str(exc)},level="error")
