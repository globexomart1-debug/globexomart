...
- Main menu uses **Products** with **Free Products** and **Paid Products**.
- Admin uploads products with name, price, duration, warranty, stock status and delivery text/files.
- Paid Products are purchased only from the user's approved in-bot USDT balance. They do not create a new manual payment request at checkout.
- Deposits require amount + transaction ID/TxID + screenshot, then admin approval. Only approved deposits increase the user's USDT balance.
- Account shows the user's current USDT balance.

## Dedicated Products Shop (latest build)

The Products section is now separate from Methods/folders and uses dedicated `shop_products` and `shop_orders` MongoDB collections.

Admin flow:
- `🆓 Upload Free Product` or `💵 Upload Paid Product`
- Enter name, price, description, duration, warranty, initial stock, and delivery note.
- Stock supports one digital item per line. Multi-line items can be separated by a line containing `---`.
- Product Manager supports edit name/price/description/duration/warranty, add/view stock, position, stockout status, hide/show, delete, orders, statistics, and bulk discounts.

User flow:
- `🛍 Products` -> Free Products / Paid Products / My Product Orders.
- User selects a product, enters quantity, confirms purchase.
- Paid products deduct only from approved USDT bot balance.
- Free products can be free or points-priced.
- Purchased stock is removed from inventory and delivered instantly.
- Out-of-stock products remain visible but cannot be purchased.

Bulk discounts can be changed by admin, for example:
`/setproductdiscounts 2=5,5=10,10=15`
